...
namespace libonvif
{
    ...
    class Session
    {
        public:
            ...
            operator OnvifSession *() { return session; }
            ...
            void discover()
            {
                if (interface.length() > 0)
                    {
                        ...
                        strcpy(
                            session->preferred_network_address,
                            interface.c_str()
                        );
                    }
                ...
                for (int i = 0; i < number_of_devices; i++)
                    {
                        ...
                        if (prepareOnvifData(i, session, data))
                            {
                                if (std::find(
                                        devices.begin(), devices.end(), data
                                    ) == devices.end())
                                    {
                        ...
                        else
                            {
                                ...
                                str << "session was unable to parse device "
                                       "onvif data: "
                                    << session->buf[i];
                ...
                std::stringstream str;
                str << " ONVIF Discovery found " << devices.size()
                    << " unique devices on network: " << interface;
                ...
                for (int i = 0; i < devices.size(); i++)
                    {
                        ...
                        if (getTimeOffset(data))
                            {
                                std::stringstream str;
                                str << "Camera get time offset error "
                                    << data.camera_name() << " : "
                                    << data.xaddrs() << " : "
                                    << data.last_error();
                                if (infoCallback) infoCallback(str.str());
            ...
            void getActiveInterfaces() { getActiveNetworkInterfaces(session); }

            std::string active_interface(int arg)
            {
                return session->active_network_interfaces[arg];
            }
            std::string primary_network_interface()
            {
                return session->primary_network_interface;
            }
