...
namespace libonvif {
    ...
    public:
        ...
        Data() { data = (OnvifData*)calloc(sizeof(OnvifData), 1); }

        Data(OnvifData* onvif_data) { data = onvif_data; }
        ...
        operator OnvifData*() { return data; }

        OnvifData* operator->() { return data; }

        void addProfile(Data& profile) { profiles.push_back(profile); }
        ...
        void updateTime()
        {
            ...
            if (last_error().length()) {
                std::stringstream str;
                str << alias << ": Set System Date and Time Error: "
                    << last_error();
                if (infoCallback)
        ...
        void stop() { moveStop(stop_type, data); }
        ...
        void move() { continuousMove(x, y, z, data); }
        ...
        {
            std::stringstream str;
            if (setVideoEncoderConfiguration(data))
                str << data->last_error << " - ";
            if (getVideoEncoderConfigurationOptions(data))
                str << data->last_error << " - ";
            if (getVideoEncoderConfiguration(data))
                str << data->last_error << " - ";
        ...
        {
            std::stringstream str;
            if (setAudioEncoderConfiguration(data))
                str << data->last_error << " - ";
            if (getAudioEncoderConfigurationOptions(data))
                str << data->last_error << " - ";
            if (getAudioEncoderConfiguration(data))
                str << data->last_error << " - ";
        ...
        {
            std::stringstream str;
            if (setImagingSettings(data))
                str << data->last_error << " - ";
            if (getOptions(data))
                str << data->last_error << " - ";
            if (getImagingSettings(data))
                str << data->last_error << " - ";
        ...
        {
            std::stringstream str;
            if (setNetworkInterfaces(data))
                str << data->last_error << " - ";
            if (setDNS(data))
                str << data->last_error << " - ";
            if (setNetworkDefaultGateway(data))
                str << data->last_error << " - ";
            std::this_thread::sleep_for(std::chrono::milliseconds(1000));
            if (getNetworkInterfaces(data))
                str << data->last_error << " - ";
            if (getNetworkDefaultGateway(data))
                str << data->last_error << " - ";
            if (getDNS(data))
                str << data->last_error << " - ";
            ...
            for (int i = 0; i < profiles.size(); i++) {
                if (strcmp(
                        profiles[i].data->profileToken,
                        data->profileToken
                    )) {
                    strcpy(
                        profiles[i].data->networkInterfaceToken,
                        data->networkInterfaceToken
                    );
                    strcpy(
                        profiles[i].data->networkInterfaceName,
                        data->networkInterfaceName
                    );
                    strcpy(
                        profiles[i].data->ip_address_buf,
                        data->ip_address_buf
                    );
                    strcpy(
                        profiles[i].data->default_gateway_buf,
                        data->default_gateway_buf
                    );
                    strcpy(
                        profiles[i].data->dns_buf, data->dns_buf
                    );
                    strcpy(
                        profiles[i].data->mask_buf, data->mask_buf
                    );
        ...
        void fill()
        {
            for (int i = 0; i < profiles.size(); i++) {
                std::stringstream str;
                if (synchronizeTime) {
                    if (setSystemDateAndTime(profiles[i]))
                        str << profiles[i]->last_error << " - ";
                }
                if (getTimeOffset(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getProfile(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getNetworkInterfaces(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getNetworkDefaultGateway(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getDNS(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getNTP(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getVideoEncoderConfiguration(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getVideoEncoderConfigurationOptions(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getAudioEncoderConfiguration(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getAudioEncoderConfigurationOptions(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getOptions(profiles[i]))
                    str << profiles[i]->last_error << " - ";
                if (getImagingSettings(profiles[i]))
                    str << profiles[i]->last_error << " - ";

                strcpy(profiles[i]->camera_name, camera_name().c_str());
                strcpy(
                    profiles[i]->device_service,
                    device_service().c_str()
                );
                ...
                int length = std::min(
                    std::max((int)(str.str().length() - 2), 0), 1024
                );
                strncpy(
                    profiles[i]->last_error, str.str().c_str(), length
                );
            }
        ...
        void manual_fill()
        {
            ...
            if (getTimeOffset(data)) {
                std::stringstream str;
                str << "Camera get time offset error " << camera_name()
                    << " : " << xaddrs() << " : " << last_error();
                if (infoCallback)
                    infoCallback(str.str());
            }
                ...
                *this = getCredential(*this);
                if (!cancelled) {
                    ...
                    if (getCapabilities(data)) {
                        // Daylight Savings Time mismatch can
                        // cause authentication failure
                        std::stringstream str;
                        str << "onvif data error "
                            << camera_name() << " : "
                            << xaddrs() << " : " << last_error()
                            << " attempting time offset "
                               "correction";
                        if (infoCallback)
                            infoCallback(str.str());
                    ...
                    if (getDeviceInformation(data) == 0) {
                        ...
                        std::stringstream str;
                        str << "get device information error: "
                            << data->camera_name << " : "
                            << data->last_error;
                        if (infoCallback)
                            infoCallback(str.str());
        ...
        std::string uri()
        {
            ...
            try {
                ...
                if (getProxyURI) {
                    ...
                    if (arg.length() > 7)
                        str << arg.substr(0, 7) << data->username
                            << ":" << data->password << "@"
                            << arg.substr(7);
                }
            } catch (const std::exception& ex) {
                std::cout << "onvif data uri() exception: " << ex.what()
                          << std::endl;
            }
        ...
        void nullifyGetProxyURI() { getProxyURI = nullptr; }

        std::string username() { return data->username; }
        const void setUsername(const std::string& arg)
        {
            memset(data->username, 0, 128);
            strncpy(data->username, arg.c_str(), arg.length());
        }

        std::string password() { return data->password; }
        const void setPassword(const std::string& arg)
        {
        ...
        std::string xaddrs() { return data->xaddrs; }
        const void setXAddrs(const std::string& arg)
        {
            strcpy(data->xaddrs, arg.c_str());
        }
        std::string device_service() { return data->device_service; }
        const void setDeviceService(const std::string& arg)
        {
            strcpy(data->device_service, arg.c_str());
        }
        std::string event_service() { return data->event_service; }
        const std::string stream_uri() { return data->stream_uri; }
        const std::string snapshot_uri() { return data->snapshot_uri; }
        const std::string serial_number() { return data->serial_number; }
        const std::string camera_name() { return data->camera_name; }
        const void setCameraName(const std::string& arg)
        {
            strcpy(data->camera_name, arg.c_str());
        }
        void setHost(const std::string& arg)
        {
            strcpy(data->host, arg.c_str());
        }
        std::string last_error() { return data->last_error; }
        const std::string profile() { return data->profileToken; }
        const void clearLastError() { memset(data->last_error, 0, 1024); }
        void setLastError(const std::string& arg)
        {
            strcpy(data->last_error, arg.c_str());
        }
        time_t time_offset() { return data->time_offset; }
        const void setTimeOffset(time_t arg) { data->time_offset = arg; }
        std::string timezone() { return data->timezone; }
        const void setTimezone(const std::string& arg)
        {
            strcpy(data->timezone, arg.c_str());
        }
        bool dst() { return data->dst; }
        const void noOp()
        {
        } // believe it or not, this line is needed to compile on windows
        ...
        const
            ...
            void
            syncProfile(int index)
        {
            if (index < profiles.size())
                copyData(profiles[index].data, data);
        }

        void syncData(const Data& other) { copyData(data, other.data); }
        ...
        // VIDEO
        std::string resolutions_buf(int arg)
        {
            return data->resolutions_buf[arg];
        }
        ...
        std::string ip_address_buf() { return data->ip_address_buf; }
        const void setIPAddressBuf(const std::string& arg)
        {
            memset(data->ip_address_buf, 0, 128);
            strncpy(data->ip_address_buf, arg.c_str(), arg.length());
        }
        std::string default_gateway_buf()
        {
            return data->default_gateway_buf;
        }
        const void setDefaultGatewayBuf(const std::string& arg)
        {
            memset(data->default_gateway_buf, 0, 128);
            strncpy(data->default_gateway_buf, arg.c_str(), arg.length());
        }
        std::string dns_buf() { return data->dns_buf; }
        const void setDNSBuf(const std::string& arg)
        {
        ...
        const void setMaskBuf(const std::string& arg)
        {
            data->prefix_length = mask2prefix((char*)arg.c_str());
        }
        std::string ntp_addr() { return data->ntp_addr; }
        const void setNTPAddr(const std::string& arg)
        {
            memset(data->ntp_addr, 0, 128);
            strncpy(data->ntp_addr, arg.c_str(), arg.length());
        }
        std::string ntp_type() { return data->ntp_type; }
        const void setNTPType(const std::string& arg)
        {
        ...
        const std::vector<int> audio_bitrates(int arg)
        {
        ...
        const std::vector<int> audio_sample_rates(int arg)
        {
            std::vector<int> result;
            for (int i = 0; i < 8; i++) {
        ...
        const std::string audio_encoding() { return data->audio_encoding; }
        const void setAudioEncoding(const std::string& arg)
        {
            memset(data->audio_encoding, 0, sizeof(data->audio_encoding));
            strncpy(data->audio_encoding, arg.c_str(), arg.length());
        }
        std::string audio_name() { return data->audio_name; }
        const int audio_bitrate() { return data->audio_bitrate; }
        ...
        std::string audio_session_timeout()
        {
            return data->audio_session_timeout;
        }
        const std::string audio_multicast_type()
        {
            return data->audio_multicast_type;
        }
        const std::string audio_multicast_address()
        {
            return data->audio_multicast_address;
        }
        const int audio_use_count() { return data->audio_use_count; }
        ...
        {
            return data->audio_multicast_auto_start;
        }
        ...
        bool Bool(bool b)
        {
            if (key == "autostart")
                data->autostart = b;
            if (key == "guaranteed_frame_rate")
                data->guaranteed_frame_rate = b;
            if (key == "dhcp_enabled")
                data->dhcp_enabled = b;
            if (key == "dst")
                data->dst = b;
            if (key == "ntp_dhcp")
                data->ntp_dhcp = b;
            if (key == "audio_multicast_auto_start")
                data->audio_multicast_auto_start = b;
            return true;
        }
        bool Int(int i)
        {
        ...
        {
            if (key == "gov_length_min")
                data->gov_length_min = i;
            if (key == "gov_length_max")
                data->gov_length_max = i;
            if (key == "frame_rate_min")
                data->frame_rate_min = i;
            if (key == "frame_rate_max")
                data->frame_rate_max = i;
            if (key == "bitrate_min")
                data->bitrate_min = i;
            if (key == "bitrate_max")
                data->bitrate_max = i;
            ...
            if (key == "use_count")
                data->use_count = i;
            if (key == "multicast_port")
                data->multicast_port = i;
            if (key == "multicast_ttl")
                data->multicast_ttl = i;
            if (key == "encoding_interval")
                data->encoding_interval = i;
            if (key == "prefix_length")
                data->prefix_length = i;
            if (key == "brightness_min")
                data->brightness_min = i;
            if (key == "brightness_max")
                data->brightness_max = i;
            if (key == "saturation_min")
                data->saturation_min = i;
            if (key == "saturation_max")
                data->saturation_max = i;
            if (key == "contrast_min")
                data->contrast_min = i;
            if (key == "contrast_max")
                data->contrast_max = i;
            if (key == "sharpness_min")
                data->sharpness_min = i;
            if (key == "sharpness_max")
                data->sharpness_max = i;
            if (key == "brightness")
                data->brightness = i;
            if (key == "saturation")
                data->saturation = i;
            ...
            if (key == "sharpness")
                data->sharpness = i;
            if (key == "event_listen_port")
                data->event_listen_port = i;
            if (key == "datetimetype")
                data->datetimetype = i;
            if (key == "audio_bitrate")
                data->audio_bitrate = i;
            if (key == "audio_sample_rate")
                data->audio_sample_rate = i;
            if (key == "audio_use_count")
                data->audio_use_count = i;
            if (key == "audio_multicast_port")
                data->audio_multicast_port = i;
            if (key == "audio_multicast_TTL")
                data->audio_multicast_TTL = i;
            if (key == "preset")
                preset = i;
            if (key == "stop_type")
                stop_type = i;
        ...
        bool
        RawNumber(const char* str, rapidjson::SizeType length, bool copy)
        {
        ...
        {
            if (key == "videoEncoderConfigurationToken")
                strncpy(data->videoEncoderConfigurationToken, str, length);
            if (key == "video_encoder_name")
                strncpy(data->video_encoder_name, str, length);
            if (key == "h264_profile")
                strncpy(data->h264_profile, str, length);
            if (key == "multicast_address_type")
                strncpy(data->multicast_address_type, str, length);
            if (key == "multicast_address")
                strncpy(data->multicast_address, str, length);
            if (key == "session_time_out")
                strncpy(data->session_time_out, str, length);
            if (key == "encoding")
                strncpy(data->encoding, str, length);
            if (key == "networkInterfaceToken")
                strncpy(data->networkInterfaceToken, str, length);
            if (key == "networkInterfaceName")
                strncpy(data->networkInterfaceName, str, length);
            if (key == "ip_address_buf")
                strncpy(data->ip_address_buf, str, length);
            if (key == "default_gateway_buf")
                strncpy(data->default_gateway_buf, str, length);
            if (key == "dns_buf")
                strncpy(data->dns_buf, str, length);
            if (key == "mask_buf")
                strncpy(data->mask_buf, str, length);
            if (key == "videoSourceConfigurationToken")
                strncpy(data->videoSourceConfigurationToken, str, length);
            if (key == "device_service")
                strncpy(data->device_service, str, length);
            if (key == "media_service")
                strncpy(data->media_service, str, length);
            if (key == "imaging_service")
                strncpy(data->imaging_service, str, length);
            if (key == "ptz_service")
                strncpy(data->ptz_service, str, length);
            if (key == "event_service")
                strncpy(data->event_service, str, length);
            if (key == "subscription_reference")
                strncpy(data->subscription_reference, str, length);
            if (key == "xaddrs")
                strncpy(data->xaddrs, str, length);
            if (key == "profileToken")
                strncpy(data->profileToken, str, length);
            if (key == "username")
                strncpy(data->username, str, length);
            if (key == "password")
                strncpy(data->password, str, length);
            if (key == "stream_uri")
                strncpy(data->stream_uri, str, length);
            if (key == "camera_name")
                strncpy(data->camera_name, str, length);
            if (key == "serial_number")
                strncpy(data->serial_number, str, length);
            if (key == "host_name")
                strncpy(data->host_name, str, length);
            if (key == "host")
                strncpy(data->host, str, length);
            if (key == "last_error")
                strncpy(data->last_error, str, length);
            if (key == "timezone")
                strncpy(data->timezone, str, length);
            if (key == "ntp_type")
                strncpy(data->ntp_type, str, length);
            if (key == "ntp_addr")
                strncpy(data->ntp_addr, str, length);
            if (key == "audio_encoding")
                strncpy(data->audio_encoding, str, length);
            if (key == "audio_name")
                strncpy(data->audio_name, str, length);
            if (key == "audioEncoderConfigurationToken")
                strncpy(data->audioEncoderConfigurationToken, str, length);
            if (key == "audioSourceConfigurationToken")
                strncpy(data->audioSourceConfigurationToken, str, length);
            if (key == "audio_session_timeout")
                strncpy(data->audio_session_timeout, str, length);
            if (key == "audio_multicast_type")
                strncpy(data->audio_multicast_type, str, length);
            if (key == "audio_multicast_address")
                strncpy(data->audio_multicast_address, str, length);

            if (key == "resolutions_buf") {
                strncpy(
                    data->resolutions_buf[counters.back()], str, length
                );
                counters.back()++;
            }
            ...
            if (key == "audio_encoders") {
                strncpy(
                    data->audio_encoders[counters.back()], str, length
                );
                counters.back()++;
            }
        ...
        /*
        Please note that this class is intended to be self contained within
        the C++ domain. It will not behave as expected if the calling python
        program attempts to extend the functionality of the class by adding
        member variables in the python domain. This was done so that the
        profile could be copied or filled with data by the C++ class
        exclusively, removing the need for additional synchronization code
        in the python domain.

        The effect of this decision is that GUI persistence for profiles
        must be implemented in this C++ class directly. The member variables
        are added to the OnvifData structure in onvif.h and the copyData and
        clearData functions in onvif.c. GUI persistence is handled by
        passing setSetting and getSetting from the calling python program
        for writing variable states to disk.
        */
        ...
        int getDesiredAspect()
        {
            ...
            str_key << serial_number() << "/" << profile()
                    << "/DesiredAspect";
            ratio
                << ((height() == 0) ? 0 : (int)(100 * width() / height()));
