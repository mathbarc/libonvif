...
public:
    ...
    friend inline void
    swap(GenericMember& a, GenericMember& b) RAPIDJSON_NOEXCEPT
    {
        a.name.Swap(b.name);
...
class GenericMemberIterator {
    ...
    typedef
        typename internal::MaybeAddConst<Const, PlainType>::Type ValueType;
...
public:
    ...
    */
    ...
    template <bool Const_>
    bool
    operator<(const GenericMemberIterator<Const_, Encoding, Allocator>& that
    ) const
    {
        return ptr_ < that.ptr_;
    }
...
namespace internal {
    ...
    struct TypeHelper<ValueType, int> {
        ...
        static ValueType&
        Set(ValueType& v, int data, typename ValueType::AllocatorType&)
        {
            return v.SetInt(data);
        }
    };
    ...
    struct TypeHelper<ValueType, long> {
        ...
        static ValueType&
        Set(ValueType& v, long data, typename ValueType::AllocatorType&)
        {
            return v.SetInt(data);
        }
    };
    ...
    struct TypeHelper<ValueType, unsigned long> {
        ...
        static ValueType&
        Set(ValueType& v, unsigned long data,
            typename ValueType::AllocatorType&)
        {
            return v.SetUint(data);
        }
    ...
    struct TypeHelper<ValueType, float> {
        ...
        static ValueType&
        Set(ValueType& v, float data, typename ValueType::AllocatorType&)
        {
            return v.SetFloat(data);
        }
    };
...
public:
    ...
    typedef
        typename GenericMemberIterator<false, Encoding, Allocator>::Iterator
            MemberIterator; //!< Member iterator for iterating in object.
    typedef
        typename GenericMemberIterator<true, Encoding, Allocator>::Iterator
            ConstMemberIterator; //!< Constant member iterator for iterating
                                 //!< in object.
    typedef GenericValue* ValueIterator; //!< Value iterator for iterating in array.
    typedef const GenericValue* ConstValueIterator; //!< Constant value iterator for iterating in
                                                    //!< array.
...
public:
        ...
        switch (rhs.GetType()) {
            ...
            case kStringType:
                if (rhs.data_.f.flags == kConstStringFlag && !copyConstStrings) {
                    ...
                    SetStringRaw(
                        StringRef(
                            rhs.GetString(), rhs.GetStringLength()
                        ),
                        allocator
                    );
                break;
    ...
    */
    ...
    {
        if (RAPIDJSON_LIKELY(this != &rhs)) {
            // Can't destroy "this" before assigning "rhs", otherwise
            // "rhs" could be used after free if it's an sub-Value of
            // "this", hence the temporary danse.
            GenericValue temp;
            temp.RawAssign(rhs);
            this->~GenericValue();
            RawAssign(temp);
    ...
    */
     ...
     */
    friend inline void
    swap(GenericValue& a, GenericValue& b) RAPIDJSON_NOEXCEPT
    {
        a.Swap(b);
    }
    ...
    */
    ...
    operator[](const GenericValue<Encoding, SourceAllocator>& name)
    {
        ...
        if (member != MemberEnd())
            ...
            // Use thread-local storage to prevent races between
            // threads. Use static buffer and placement-new to prevent
            // destruction, with alignas() to ensure proper alignment.
            alignas(GenericValue
            ) thread_local static char buffer[sizeof(GenericValue)];
            return *new (buffer) GenericValue();
#elif defined(_MSC_VER) && _MSC_VER < 1900
            // There's no way to solve both thread locality and proper
            // alignment simultaneously.
            __declspec(thread) static char buffer[sizeof(GenericValue)];
            return *new (buffer) GenericValue();
#elif defined(__GNUC__) || defined(__clang__)
                                             // This will generate
                                             // -Wexit-time-destructors in
                                             // clang,
                    // but that's better than having under-alignment.
                    __thread static GenericValue buffer;
                    return buffer;
#else
                                             // Don't know what compiler
                                             // this is, so don't know how
                    // to ensure thread-locality.
    ...
    */
    GenericValue& AddMember(
        GenericValue& name, std::basic_string<Ch>& value,
        Allocator& allocator
    )
    {
    ...
    */
    GenericValue&
    SetString(const std::basic_string<Ch>& s, Allocator& allocator)
    {
        return SetString(StringRef(s), allocator);
    }
#endif
        ...
        switch (GetType()) {
            ...
            case kObjectType:
                ...
                for (ConstMemberIterator m = MemberBegin();
                    ...
                    ); // User may change the type of name by
                       // MemberIterator.
...
private:
    ...
    struct Flag {
        ...
        char payload[sizeof(SizeType) * 2 + 6]; // 2 x SizeType + lower
                                                // 48-bit pointer
#elif RAPIDJSON_64BIT
        char payload
            [sizeof(SizeType) * 2 + sizeof(void*) + 6]; // 6 padding bytes
#else
                char payload
                    [sizeof(SizeType) * 2 + sizeof(void*) + 2]; // 2 padding bytes
#endif
        uint16_t flags;
    };
    ...
    SetElementsPointer(GenericValue* elements)
    {
        return RAPIDJSON_SETPOINTER(
            GenericValue, data_.a.elements, elements
        );
    }
    ...
    struct MapTraits {
        struct Less {
            ...
            {
                SizeType n1 = DataStringLength(s1),
                         n2 = DataStringLength(s2);
                int cmp = std::memcmp(
                    DataString(s1), DataString(s2),
        ...
        typedef std::multimap<
            Data, SizeType, Less, StdAllocator<Pair, Allocator>>
            Map;
    ...
    //
    // Layout of the members' map/array, re(al)located according to the
    // needed capacity:
    //
    ...
    {
        return *reinterpret_cast<SizeType*>(
    ...
    static RAPIDJSON_FORCEINLINE MapIterator* GetMapIterators(Map*& map)
    {
        return reinterpret_cast<MapIterator*>(
            reinterpret_cast<uintptr_t>(&map) + RAPIDJSON_ALIGN(sizeof(Map*)) + RAPIDJSON_ALIGN(sizeof(SizeType)) + RAPIDJSON_ALIGN(GetMapCapacity(map) * sizeof(Member))
        );
    }
    ...
    // Some compilers' debug mechanisms want all iterators to be destroyed,
    ...
    Map*&
    DoReallocMap(Map** oldMap, SizeType newCapacity, Allocator& allocator)
    {
        ...
        if (!oldMap) {
            *newMap = new (allocator.Malloc(sizeof(Map)))
                Map(MapLess(), allocator);
        } else {
            *newMap = *oldMap;
            ...
            while (count--) {
                new (&newIt[count])
                    MapIterator(DropMapIterator(oldIt[count]));
            }
    ...
    DoAllocMembers(SizeType capacity, Allocator& allocator)
    {
    ...
    )
    {
        ...
        if (o.size >= o.capacity)
            DoReserveMembers(
                o.capacity ? (o.capacity + (o.capacity + 1) / 2)
    ...
    MemberIterator
    DoEraseMembers(ConstMemberIterator first, ConstMemberIterator last)
    {
        ObjectData& o = data_.o;
        ...
        if (first != last) {
            ...
            for (MemberIterator itr = pos; next != end; ++itr, ++next) {
                std::memcpy(
                    static_cast<void*>(&*itr), &*next,
                    sizeof(Member)
                );
                SizeType mpos = static_cast<SizeType>(itr - beg);
                new (&mit[mpos])
                    MapIterator(DropMapIterator(mit[next - beg]));
                mit[mpos]->second = mpos;
            }
        }
#else
    ...
    void SetObjectRaw(Member* members, SizeType count, Allocator& allocator)
    {
        ...
        if (count) {
            ...
            SetMembersPointer(m);
            std::memcpy(
                static_cast<void*>(m), members, count * sizeof(Member)
            );
#if RAPIDJSON_USE_MEMBERSMAP
            Map*& map = GetMap(m);
            MapIterator* mit = GetMapIterators(map);
            for (SizeType i = 0; i < count; i++) {
                new (&mit[i]) MapIterator(
                    map->insert(MapPair(m[i].name.data_, i))
                );
            }
#endif
        } else
    ...
    template <typename SourceAllocator>
    bool StringEqual(const GenericValue<Encoding, SourceAllocator>& rhs
    ) const
    {
        RAPIDJSON_ASSERT(IsString());
...
public:
    ...
    typedef Allocator
        AllocatorType; //!< Allocator type from template parameter.
    typedef StackAllocator StackAllocatorType; //!< StackAllocator type from
                                               //!< template parameter.
    ...
    */
    ...
    ~GenericDocument()
    {
        // Clear the ::ValueType before ownAllocator is destroyed,
        // ~ValueType() runs last and may access its elements or members
        // which would be freed with an allocator like MemoryPoolAllocator
        // (CrtAllocator does not free its data when destroyed, but
        // MemoryPoolAllocator does).
        if (ownAllocator_) {
            ValueType::SetNull();
        }
        Destroy();
    ...
    */
     ...
     */
    friend inline void
    swap(GenericDocument& a, GenericDocument& b) RAPIDJSON_NOEXCEPT
    {
        a.Swap(b);
    }
    ...
    */
    template <
        unsigned parseFlags, typename SourceEncoding, typename InputStream>
...
public:
    ...
    GenericArray
    Reserve(SizeType newCapacity, AllocatorType& allocator) const
    {
...
#endif // RAPIDJSON_HAS_CXX11_RVALUE_REFS
    GenericArray
    PushBack(StringRefType value, AllocatorType& allocator) const
    {
        value_.PushBack(value, allocator);
        return *this;
    }
    ...
    ValueIterator
    Erase(ConstValueIterator first, ConstValueIterator last) const
    {
...
public:
    ...
    GenericObject
    MemberReserve(SizeType newCapacity, AllocatorType& allocator) const
    {
    ...
    GenericObject AddMember(
        ValueType& name, ValueType& value, AllocatorType& allocator
    ) const
    {
    ...
    GenericObject AddMember(
        ValueType& name, StringRefType value, AllocatorType& allocator
    ) const
    {
        value_.AddMember(name, value, allocator);
        return *this;
    ...
    GenericObject AddMember(
        StringRefType name, ValueType&& value, AllocatorType& allocator
    ) const
    {
        value_.AddMember(name, value, allocator);
        return *this;
    ...
    GenericObject AddMember(
        StringRefType name, StringRefType value, AllocatorType& allocator
    ) const
    {
        value_.AddMember(name, value, allocator);
        return *this;
