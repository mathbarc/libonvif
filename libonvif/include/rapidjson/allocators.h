...
class CrtAllocator
{
    public:
        ...
        void *Malloc(size_t size)
        {
            ...
            else return NULL; // standardize to returning NULL.
        }
        void *Realloc(void *originalPtr, size_t originalSize, size_t newSize)
        {
            (void)originalSize;
...
template <typename BaseAllocator = CrtAllocator> class MemoryPoolAllocator
{
        ...
        struct ChunkHeader
        {
                size_t capacity; //!< Capacity of the chunk in bytes (excluding
                                 //!< the header itself).
        ...
        struct SharedData
        {
                ChunkHeader *chunkHead; //!< Head of the chunk linked-list. Only
                                        //!< the head chunk serves allocation.
                BaseAllocator *ownBaseAllocator; //!< base allocator created by
                                                 //!< this object.
                size_t refcount;
                bool ownBuffer;
        };
        ...
        static const size_t kDefaultChunkCapacity =
            RAPIDJSON_ALLOCATOR_DEFAULT_CHUNK_CAPACITY; //!< Default chunk
                                                        //!< capacity.
    ...
    public:
        ...
        static const bool kRefCounted =
            true; //!< Tell users that this allocator is reference counted on
                  //!< copy
        ...
        */
        explicit MemoryPoolAllocator(
            ...
            : chunk_capacity_(chunkSize),
              baseAllocator_(
                  baseAllocator ? baseAllocator : RAPIDJSON_NEW(BaseAllocator)()
              ),
              shared_(static_cast<SharedData *>(
                  baseAllocator_ ? baseAllocator_->Malloc(
                                       SIZEOF_SHARED_DATA + SIZEOF_CHUNK_HEADER
                                   )
                                 : 0
              ))
        {
            ...
            if (baseAllocator) { shared_->ownBaseAllocator = 0; }
            else { shared_->ownBaseAllocator = baseAllocator_; }
        ...
        /*! The user buffer will be used firstly. When it is full, memory pool
           ...
           than sizeof(ChunkHeader). \param chunkSize The size of memory chunk.
           The default is kDefaultChunkSize. \param baseAllocator The allocator
           for allocating memory chunks.
        */
        MemoryPoolAllocator(
        ...
        MemoryPoolAllocator &operator=(MemoryPoolAllocator &&rhs
        ) RAPIDJSON_NOEXCEPT
        {
            RAPIDJSON_NOEXCEPT_ASSERT(rhs.shared_->refcount > 0);
            this->~MemoryPoolAllocator();
        ...
        //! Destructor.
        /*! This deallocates all memory chunks, excluding the user-supplied
         * buffer.
         */
        ~MemoryPoolAllocator() RAPIDJSON_NOEXCEPT
        {
            ...
            if (shared_->ownBuffer) { baseAllocator_->Free(shared_); }
            RAPIDJSON_DELETE(a);
        }
        ...
        void *Realloc(void *originalPtr, size_t originalSize, size_t newSize)
        {
            ...
            // sufficient space
            if (originalPtr == GetChunkBuffer(shared_) +
...
class StdAllocator : public std::allocator<T>
{
    ...
    public:
        ...
        template <typename U>
        StdAllocator(const StdAllocator<U, BaseAllocator> &rhs
        ) RAPIDJSON_NOEXCEPT : allocator_type(rhs),
                               baseAllocator_(rhs.baseAllocator_)
        {
        }
