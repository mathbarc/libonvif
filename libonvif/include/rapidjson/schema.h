...
namespace internal
{
    ...
    inline void
    PrintValidatorPointersData(const char *s, const char *d, unsigned depth)
    {
        printf(
            "    Sch: %*s'%s'\n    Doc: %*s'%s'\n", depth * 4, " ", s,
            depth * 4, " ", d
    ...
    inline void PrintValidatorPointersData(
        const wchar_t *s, const wchar_t *d, unsigned depth
    )
    {
        wprintf(
            L"    Sch: %*ls'%ls'\n    Doc: %*ls'%ls'\n", depth * 4, L" ", s,
    ...
    inline void PrintSchemaIdsData(
        const char *base, const char *local, const char *resolved
    )
    {
        printf(
    ...
    inline void PrintSchemaIdsData(
        const wchar_t *base, const wchar_t *local, const wchar_t *resolved
    )
    {
        wprintf(
    ...
    inline void
    PrintMethodData(const char *method, const char *s1, const char *s2)
    {
        printf("%s, Data: '%s', '%s'\n", method, s1, s2);
    }

    inline void
    PrintMethodData(const char *method, const wchar_t *s1, const wchar_t *s2)
    {
...
struct Specification
{
        ...
        {
            return (
                (draft >= kDraftMin && draft <= kDraftMax) &&
                ((oapi == kVersionNone) ||
...
namespace internal
{
    ...
    template <typename SchemaType> class ISchemaStateFactory
    {
        public:
            ...
            virtual ISchemaValidator *CreateSchemaValidator(
                const SchemaType &, const bool inheritContinueOnErrors
            ) = 0;
    ...
    template <typename SchemaType> class IValidationErrorHandler
    {
        public:
            ...
            virtual void
            NotMultipleOf(int64_t actual, const SValue &expected) = 0;
            virtual void
            NotMultipleOf(uint64_t actual, const SValue &expected) = 0;
            virtual void
            NotMultipleOf(double actual, const SValue &expected) = 0;
            virtual void AboveMaximum(
                int64_t actual, const SValue &expected, bool exclusive
            ) = 0;
            virtual void AboveMaximum(
                uint64_t actual, const SValue &expected, bool exclusive
            ) = 0;
            virtual void AboveMaximum(
            ...
            virtual void
            TooLong(const Ch *str, SizeType length, SizeType expected) = 0;
            virtual void
            TooShort(const Ch *str, SizeType length, SizeType expected) = 0;
            virtual void DoesNotMatch(const Ch *str, SizeType length) = 0;

            virtual void DisallowedItem(SizeType index) = 0;
            virtual void
            TooFewItems(SizeType actualCount, SizeType expectedCount) = 0;
            virtual void
            TooManyItems(SizeType actualCount, SizeType expectedCount) = 0;
            virtual void DuplicateItems(SizeType index1, SizeType index2) = 0;

            virtual void
            TooManyProperties(SizeType actualCount, SizeType expectedCount) = 0;
            virtual void
            TooFewProperties(SizeType actualCount, SizeType expectedCount) = 0;
            ...
            virtual void
            DisallowedProperty(const Ch *name, SizeType length) = 0;
            ...
            virtual void AddDependencySchemaError(
                const SValue &souceName, ISchemaValidator *subvalidator
            ) = 0;
            ...
            virtual void
            NotAllOf(ISchemaValidator **subvalidators, SizeType count) = 0;
            virtual void
            NoneOf(ISchemaValidator **subvalidators, SizeType count) = 0;
            virtual void
            NotOneOf(ISchemaValidator **subvalidators, SizeType count) = 0;
    ...
    template <typename Encoding, typename Allocator> class Hasher
    {
        public:
            ...
            Hasher(
                Allocator *allocator = 0, size_t stackCapacity = kDefaultSize
            )
            ...
            bool EndObject(SizeType memberCount)
            {
                ...
                for (SizeType i = 0; i < memberCount; i++)
                    // Issue #2205
                    // Hasing the key to avoid key=value cases with bug-prone
                    // zero-value hash
                    h ^= Hash(
                        Hash(0, kv[i * 2]), kv[i * 2 + 1]
                    ); // Use xor to achieve member order insensitive
            ...
            bool EndArray(SizeType elementCount)
            {
                ...
                for (SizeType i = 0; i < elementCount; i++)
                    h = Hash(
                        h, e[i]
                    ); // Use hash to achieve element order sensitive
                *stack_.template Push<uint64_t>() = h;
                return true;
            }
        ...
        private:
            ...
            bool WriteBuffer(Type type, const void *data, size_t len)
            {
                ...
                uint64_t h =
                    Hash(RAPIDJSON_UINT64_C2(0xcbf29ce4, 0x84222325), type);
    ...
    template <typename SchemaDocumentType> struct SchemaValidationContext
    {
            ...
            SchemaValidationContext(
                SchemaValidatorFactoryType &f, ErrorHandlerType &eh,
                const SchemaType *s, unsigned fl = 0
            )
                : factory(f), error_handler(eh), schema(s), flags(fl),
                  valueSchema(), invalidKeyword(), invalidCode(), hasher(),
                  ...
                  propertyExist(), inArray(false), valueUniqueness(false),
            ...
            ~SchemaValidationContext()
            {
                        ...
                        for (SizeType i = 0; i < validatorCount; i++)
                            {
                                if (validators[i])
                                    {
                                        factory.DestroySchemaValidator(
                                            validators[i]
                                        );
                                    }
                            }
                ...
                if (patternPropertiesValidators)
                    {
                        for (SizeType i = 0;
                             i < patternPropertiesValidatorCount; i++)
                            {
        ...
        public:
            ...
            Schema(
                ...
                : allocator_(allocator),
                  ...
                  typeless_(schemaDocument->GetTypeless()), enum_(),
                  enumCount_(), not_(),
                  ...
                  minLength_(0), maxLength_(~SizeType(0)),
                  exclusiveMinimum_(false), exclusiveMaximum_(false),
                  defaultValueLength_(0), readOnly_(false), writeOnly_(false),
                  nullable_(false)
            {
                GenericStringBuffer<EncodingType> sb;
                p.StringifyUriFragment(sb);
                RAPIDJSON_SCHEMA_PRINT(
                    Method, "Schema::Schema", sb.GetString(), id.GetString()
                );

                typedef
                    typename ValueType::ConstValueIterator ConstValueIterator;
                typedef
                    typename ValueType::ConstMemberIterator ConstMemberIterator;

                // PR #1393
                // Early add this Schema and its $ref(s) in schemaDocument's map
                // to avoid infinite recursion (with recursive schemas), since
                // schemaDocument->getSchema() is always checked before creating
                // a new one. Don't cache typeless_, though.
                if (this != typeless_)
                    {
                        typedef typename SchemaDocumentType::SchemaEntry
                            SchemaEntry;
                        SchemaEntry *entry = schemaDocument->schemaMap_
                                                 .template Push<SchemaEntry>();
                        new (entry)
                            SchemaEntry(pointer_, this, true, allocator_);
                            ...
                            if (v->IsString())
                                {
                                    ...
                                    RAPIDJSON_SCHEMA_PRINT(
                                        SchemaIds, id.GetString(),
                                        v->GetString(), id_.GetString()
                                    );
                                }
                        }
                ...
                if (const ValueType *v = GetMember(value, GetTypeString()))
                    {
                        ...
                        if (v->IsString()) AddType(*v);
                            ...
                            for (ConstValueIterator itr = v->Begin();
                                 itr != v->End(); ++itr)
                                AddType(*itr);
                    }
                ...
                if (const ValueType *v = GetMember(value, GetEnumString()))
                    {
                        if (v->IsArray() && v->Size() > 0)
                            {
                                enum_ =
                                    static_cast<uint64_t *>(allocator_->Malloc(
                ...
                if (schemaDocument)
                    AssignIfExist(
                        allOf_, *schemaDocument, p, value, GetAllOfString(),
                        document
                    );
                ...
                if (schemaDocument && spec_.oapi != kVersion20)
                    {
                        AssignIfExist(
                            anyOf_, *schemaDocument, p, value, GetAnyOfString(),
                            document
                        );
                        AssignIfExist(
                            oneOf_, *schemaDocument, p, value, GetOneOfString(),
                            document
                        );
                        ...
                        if (const ValueType *v =
                                GetMember(value, GetNotString()))
                            {
                                schemaDocument->CreateSchema(
                                    &not_, p.Append(GetNotString(), allocator_),
                                    *v, document, id_
                                );
                    ...
                    GetMember(value, GetDependenciesString());
                {
                    ...
                    if (properties && properties->IsObject())
                        for (ConstMemberIterator itr =
                                 properties->MemberBegin();
                    ...
                    if (spec_.oapi != kVersion20 && spec_.oapi != kVersion30)
                        if (dependencies && dependencies->IsObject())
                            for (ConstMemberIterator itr =
                                     dependencies->MemberBegin();
                                 itr != dependencies->MemberEnd(); ++itr)
                                {
                                    AddUniqueElement(allProperties, itr->name);
                                    if (itr->value.IsArray())
                                        for (ConstValueIterator i =
                                                 itr->value.Begin();
                                             i != itr->value.End(); ++i)
                                            if (i->IsString())
                                                AddUniqueElement(
                    ...
                    if (allProperties.Size() > 0)
                        {
                            ...
                            properties_ =
                                static_cast<Property *>(allocator_->Malloc(
                ...
                if (spec_.oapi != kVersion20 && spec_.oapi != kVersion30)
                    if (const ValueType *v =
                        ...
                        {
                            PointerType q = p.Append(
                                GetPatternPropertiesString(), allocator_
                            );
                            ...
                            for (ConstMemberIterator itr = v->MemberBegin();
                                 itr != v->MemberEnd(); ++itr)
                                {
                                    new (&patternProperties_
                                             [patternPropertyCount_])
                                        PatternProperty();
                                    PointerType r =
                                        q.Append(itr->name, allocator_);
                                    patternProperties_[patternPropertyCount_]
                    ...
                    if (dependencies && dependencies->IsObject())
                        {
                            ...
                            for (ConstMemberIterator itr =
                                 ...
                                 itr != dependencies->MemberEnd(); ++itr)
                                {
                                    ...
                                    if (FindPropertyIndex(
                                            itr->name, &sourceIndex
                                        ))
                                        {
                                            if (itr->value.IsArray())
                                                {
                        ...
                        GetMember(value, GetAdditionalPropertiesString()))
                    {
                        if (v->IsBool()) additionalProperties_ = v->GetBool();
                            ...
                            schemaDocument->CreateSchema(
                                ...
                                p.Append(
                                    GetAdditionalPropertiesString(), allocator_
                                ),
                                *v, document, id_
                            );
                    }
                            ...
                            GetMember(value, GetAdditionalItemsString()))
                        {
                            if (v->IsBool()) additionalItems_ = v->GetBool();
                                ...
                                schemaDocument->CreateSchema(
                                    ...
                                    p.Append(
                                        GetAdditionalItemsString(), allocator_
                                    ),
                                    *v, document, id_
                                );
                        }
                ...
                AssignIfExist(
                    exclusiveMinimum_, value, GetExclusiveMinimumString()
                );
                AssignIfExist(
                    exclusiveMaximum_, value, GetExclusiveMaximumString()
                );
                ...
                // Default
                if (const ValueType *v =
                        GetMember(value, GetDefaultValueString()))
                    if (v->IsString())
                ...
                if (readOnly_ && writeOnly_)
                    schemaDocument->SchemaError(
                        kSchemaErrorReadOnlyAndWriteOnly, p
                    );
            ...
            RAPIDJSON_FORCEINLINE bool EndValue(Context &context) const
            {
                ...
                if (context.patternPropertiesValidatorCount > 0)
                    {
                        ...
                        SizeType count =
                            context.patternPropertiesValidatorCount;
                        ...
                        for (SizeType i = 0; i < count; i++)
                            if (!context.patternPropertiesValidators[i]
                                     ->IsValid())
                                {
                                    patternValid = false;
                                    break;
                ...
                if (enum_ && context.hasher)
                    {
                        const uint64_t h =
                            context.factory.GetHashCode(context.hasher);
                ...
                if (context.validatorCount > 0)
                    {
                        if (allOf_.schemas)
                            for (SizeType i = allOf_.begin;
                                 i < allOf_.begin + allOf_.count; i++)
                                if (!context.validators[i]->IsValid())
                                    {
                                        context.error_handler.NotAllOf(
                                            &context.validators[allOf_.begin],
                                            allOf_.count
                                        );
                                        RAPIDJSON_INVALID_KEYWORD_RETURN(
                                            kValidateErrorAllOf
                        ...
                        if (anyOf_.schemas)
                            {
                                for (SizeType i = anyOf_.begin;
                                     i < anyOf_.begin + anyOf_.count; i++)
                                    if (context.validators[i]->IsValid())
                                        goto foundAny;
                                context.error_handler.NoneOf(
                                    &context.validators[anyOf_.begin],
                                    anyOf_.count
                                );
                                RAPIDJSON_INVALID_KEYWORD_RETURN(
                                    kValidateErrorAnyOf
                        ...
                        if (oneOf_.schemas)
                            {
                                ...
                                for (SizeType i = oneOf_.begin;
                                     i < oneOf_.begin + oneOf_.count; i++)
                                    if (context.validators[i]->IsValid())
                                        {
                                            if (oneValid)
                                                {
            ...
            bool
            String(Context &context, const Ch *str, SizeType length, bool) const
            {
                ...
                if (minLength_ != 0 || maxLength_ != SizeType(~0))
                    {
                        ...
                        if (internal::CountStringCodePoint<EncodingType>(
                            ...
                            ))
                            {
                                if (count < minLength_)
                                    {
                                        context.error_handler.TooShort(
                                            str, length, minLength_
                                        );
                                        RAPIDJSON_INVALID_KEYWORD_RETURN(
                                ...
                                if (count > maxLength_)
                                    {
                                        context.error_handler.TooLong(
                                            str, length, maxLength_
                                        );
                                        RAPIDJSON_INVALID_KEYWORD_RETURN(
            ...
            bool StartObject(Context &context) const
            {
                ...
                if (hasDependencies_ || hasRequired_)
                    {
                        context.propertyExist =
                            static_cast<bool *>(context.factory.MallocState(
                ...
                if (patternProperties_)
                    { // pre-allocate schema array
                        ...
                        std::memset(
                            context.patternPropertiesSchemas, 0,
                ...
                if (patternProperties_)
                    {
                        ...
                        for (SizeType i = 0; i < patternPropertyCount_; i++)
                            if (patternProperties_[i].pattern &&
                                IsPatternMatch(
                                    patternProperties_[i].pattern, str, len
                                ))
                                {
                                    context.patternPropertiesSchemas
                                        [context
                        ...
                        if (context.patternPropertiesSchemaCount > 0)
                            {
                                context.patternPropertiesSchemas
                                    ...
                                    additionalPropertiesSchema_;
                                context.valueSchema = typeless_;
                                context.valuePatternValidatorType = Context::
                                    kPatternValidatorWithAdditionalProperty;
                            }
            ...
            bool EndObject(Context &context, SizeType memberCount) const
            {
                ...
                if (hasRequired_)
                    {
                        ...
                        for (SizeType index = 0; index < propertyCount_;
                             index++)
                            if (properties_[index].required &&
                                !context.propertyExist[index])
                                if (properties_[index]
                                        .schema->defaultValueLength_ == 0)
                ...
                if (memberCount < minProperties_)
                    {
                        context.error_handler.TooFewProperties(
                ...
                if (memberCount > maxProperties_)
                    {
                        context.error_handler.TooManyProperties(
                ...
                if (hasDependencies_)
                    {
                        ...
                        for (SizeType sourceIndex = 0;
                             sourceIndex < propertyCount_; sourceIndex++)
                            {
            ...
            bool StartArray(Context &context) const
            {
                ...
                context.inArray =
                    true; // Ensure we note that we are in an array
            ...
            bool EndArray(Context &context, SizeType elementCount) const
            {
                ...
                if (elementCount < minItems_)
                    {
                        context.error_handler.TooFewItems(
                            elementCount, minItems_
                        );
                ...
                if (elementCount > maxItems_)
                    {
                        context.error_handler.TooManyItems(
                            elementCount, maxItems_
                        );
...
#define RAPIDJSON_STRING_(name, ...)                                           \
    ...
    {                                                                          \
        ...
        return v;                                                              \
            ...
            RAPIDJSON_STRING_(
                Dependencies, 'd', 'e', 'p', 'e', 'n', 'd', 'e', 'n', 'c', 'i',
                'e', 's'
            )
            RAPIDJSON_STRING_(
                PatternProperties, 'p', 'a', 't', 't', 'e', 'r', 'n', 'P', 'r',
                'o', 'p', 'e', 'r', 't', 'i', 'e', 's'
            )
            RAPIDJSON_STRING_(
                AdditionalProperties, 'a', 'd', 'd', 'i', 't', 'i', 'o', 'n',
                'a', 'l', 'P', 'r', 'o', 'p', 'e', 'r', 't', 'i', 'e', 's'
            )
            RAPIDJSON_STRING_(
                MinProperties, 'm', 'i', 'n', 'P', 'r', 'o', 'p', 'e', 'r', 't',
                'i', 'e', 's'
            )
            RAPIDJSON_STRING_(
                MaxProperties, 'm', 'a', 'x', 'P', 'r', 'o', 'p', 'e', 'r', 't',
                'i', 'e', 's'
            )
            ...
            RAPIDJSON_STRING_(
                AdditionalItems, 'a', 'd', 'd', 'i', 't', 'i', 'o', 'n', 'a',
                'l', 'I', 't', 'e', 'm', 's'
            )
            RAPIDJSON_STRING_(
                UniqueItems, 'u', 'n', 'i', 'q', 'u', 'e', 'I', 't', 'e', 'm',
                's'
            )
            RAPIDJSON_STRING_(
            ...
            RAPIDJSON_STRING_(
                ExclusiveMinimum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v', 'e',
                'M', 'i', 'n', 'i', 'm', 'u', 'm'
            )
            RAPIDJSON_STRING_(
                ExclusiveMaximum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v', 'e',
                'M', 'a', 'x', 'i', 'm', 'u', 'm'
            )
            RAPIDJSON_STRING_(
...
#undef RAPIDJSON_STRING_
        ...
        private:
...
#if RAPIDJSON_SCHEMA_USE_INTERNALREGEX
            typedef internal::GenericRegex<EncodingType, AllocatorType>
                RegexType;
#elif RAPIDJSON_SCHEMA_USE_STDREGEX
            ...
            {
                for (typename V1::ConstValueIterator itr = a.Begin();
                     itr != a.End(); ++itr)
                    if (*itr == v) return;
                V1 c(v, *allocator_);
                        ...
                        if (v->IsArray() && v->Size() > 0)
                            {
                                ...
                                memset(
                                    out.schemas, 0, sizeof(Schema *) * out.count
                                );
                                for (SizeType i = 0; i < out.count; i++)
                                    schemaDocument.CreateSchema(
                                        &out.schemas[i],
                                        q.Append(i, allocator_), (*v)[i],
                                        document, id_
                                    );
            ...
            )
            {
                if (value.IsString())
                    {
                        RegexType *r =
                            new (allocator_->Malloc(sizeof(RegexType)))
                                RegexType(value.GetString(), allocator_);
                        if (!r->IsValid())
                            {
                                sd->SchemaErrorValue(
                                    kSchemaErrorRegexInvalid, p,
            ...
            )
            {
                if (value.IsString())
                    {
                        RegexType *r = static_cast<RegexType *>(
                            allocator_->Malloc(sizeof(RegexType))
                        );
                        try
                            {
            ...
            void AddType(const ValueType &type)
            {
                if (type == GetNullString()) type_ |= 1 << kNullSchemaType;
                    ...
                    type_ |=
                        (1 << kNumberSchemaType) | (1 << kIntegerSchemaType);
            }

            // Creates parallel validators for allOf, anyOf, oneOf, not and
            // schema dependencies, if required. Also creates a hasher for enums
            // and array uniqueness, if required. Also a useful place to add
            // type-independent error checks.
                ...
                if (validatorCount_)
                    {
                        ...
                        // Always return after first failure for these
                        // sub-validators
                        if (allOf_.schemas)
            ...
            void CreateSchemaValidators(
                Context &context, const SchemaArray &schemas,
            ...
            // O(n)
            bool
            FindPropertyIndex(const ValueType &name, SizeType *outIndex) const
            {
            ...
            {
                if (!(type_ &
                      ((1 << kIntegerSchemaType) | (1 << kNumberSchemaType))))
                    {
                        ...
                        if (minimum_.IsInt64())
                            {
                                if (exclusiveMinimum_ ? i <= minimum_.GetInt64()
                                                      : i < minimum_.GetInt64())
                                    {
                                        context.error_handler.BelowMinimum(
                                            i, minimum_, exclusiveMinimum_
                                        );
                                        RAPIDJSON_INVALID_KEYWORD_RETURN(
                                            exclusiveMinimum_
                ...
                if (!maximum_.IsNull())
                    {
                        if (maximum_.IsInt64())
                            {
                                if (exclusiveMaximum_ ? i >= maximum_.GetInt64()
                                                      : i > maximum_.GetInt64())
                                    {
                                        context.error_handler.AboveMaximum(
                                            i, maximum_, exclusiveMaximum_
                                        );
                                        RAPIDJSON_INVALID_KEYWORD_RETURN(
                                            exclusiveMaximum_
                        ...
                        /* do nothing */ // i <= max(int64_t) <
                                         // maximum_.GetUint64()
                        else if (!CheckDoubleMaximum(
                                     context, static_cast<double>(i)
                                 ))
                            return false;
                    }
                        ...
                        if (multipleOf_.IsUint64())
                            {
                                if (static_cast<uint64_t>(i >= 0 ? i : -i) %
                                    ...
                                    0)
                                    {
                                        context.error_handler.NotMultipleOf(
                                            i, multipleOf_
                                        );
                                        RAPIDJSON_INVALID_KEYWORD_RETURN(
            ...
            void
            DisallowedType(Context &context, const ValueType &actualType) const
            {
            ...
            struct Property
            {
                    Property()
                        : schema(), dependenciesSchema(),
                          dependenciesValidatorIndex(), dependencies(),
                          required(false)
                    {
                    }
    ...
    {
            RAPIDJSON_FORCEINLINE static void
    ...
    template <typename Stack> struct TokenHelper<Stack, char>
    {
            ...
            AppendIndexToken(Stack &documentStack, SizeType index)
            {
                if (sizeof(SizeType) == 4)
                    {
                        char *buffer = documentStack.template Push<char>(
                            1 + 10
                        ); // '/' + uint
                        *buffer++ = '/';
    ...
    public:
        ...
        )
        {
            ...
            return GetRemoteDocument(
                uri.GetBaseString(), uri.GetBaseStringLength()
            );
        }
};
...
class GenericSchemaDocument
{
    public:
        ...
        /*!
           ...
           violation reporting. \param uriLength Length of \c name, in code
           points. \param remoteProvider An optional remote schema document
           provider for resolving remote reference. Can be null. \param
           allocator An optional allocator instance for allocating memory. Can
           be null. \param pointer An optional JSON pointer to the start of the
           schema document \param spec Optional schema draft or OpenAPI version.
           Used if no specification in document. Defaults to draft-04.
        */
        explicit GenericSchemaDocument(
            const ValueType &document, const Ch *uri = 0,
        ...
        {
            RAPIDJSON_SCHEMA_PRINT(
                Method, "GenericSchemaDocument::GenericSchemaDocument"
            );
            if (!allocator_)
            ...
            // We only ever look for '$schema' or 'swagger' or 'openapi' at the
            // root of the document.
            SetSchemaSpecification(document);

            // Generate root schema, it will call CreateSchema() to create
            // sub-schemas, And call HandleRefSchema() if there are $ref. PR
            // #1393 use input pointer if supplied
            root_ = typeless_;
            if (pointer.GetTokenCount() == 0)
            ...
            else
                {
                    ...
                    SchemaErrorValue(
                        kSchemaErrorStartUnknown, PointerType(), sb.GetString(),
        ...
        static const Specification GetSpecification(const ValueType &document)
        {
            ...
            if (draft != kDraftNone) return Specification(draft);
            else
                {
        ...
        void
        SchemaError(const SchemaErrorCode code, const PointerType &location)
        {
        ...
        void SchemaErrorValue(
            const SchemaErrorCode code, const PointerType &location,
            const Ch *value, SizeType length
        )
        {
            currentError_ = GValue(kObjectType);
            currentError_.AddMember(
                GetValueString(), GValue(value, length, *allocator_).Move(),
                *allocator_
            );
            AddCurrentError(code, location);
        }
        ...
        )
        {
            ...
            currentError_.AddMember(
                ...
                static_cast<SizeType>(
                    pointer.GetParseErrorOffset() / sizeof(Ch)
                ),
                *allocator_
            );
            AddCurrentError(code, location);
    ...
    private:
        ...
        {
                SchemaEntry(
                    const PointerType &p, SchemaType *s, bool o,
                    Allocator *allocator
                )
        ...
        void
        AddErrorInstanceLocation(GValue &result, const PointerType &location)
        {
            GenericStringBuffer<EncodingType> sb;
            location.StringifyUriFragment(sb);
            GValue instanceRef(
                sb.GetString(),
                static_cast<SizeType>(sb.GetSize() / sizeof(Ch)), *allocator_
            );
        ...
        void
        AddCurrentError(const SchemaErrorCode code, const PointerType &location)
        {
...
#define RAPIDJSON_STRING_(name, ...)                                           \
    ...
    {                                                                          \
        ...
        RAPIDJSON_STRING_(Null, 'n', 'u', 'l', 'l')
        RAPIDJSON_STRING_(
            SpecUnknown, 'S', 'p', 'e', 'c', 'U', 'n', 'k', 'n', 'o', 'w', 'n'
        )
        RAPIDJSON_STRING_(
            SpecUnsupported, 'S', 'p', 'e', 'c', 'U', 'n', 's', 'u', 'p', 'p',
            'o', 'r', 't', 'e', 'd'
        )
        RAPIDJSON_STRING_(
            SpecIllegal, 'S', 'p', 'e', 'c', 'I', 'l', 'l', 'e', 'g', 'a', 'l'
        )
        RAPIDJSON_STRING_(
            StartUnknown, 'S', 't', 'a', 'r', 't', 'U', 'n', 'k', 'n', 'o', 'w',
            'n'
        )
        RAPIDJSON_STRING_(
            RefPlainName, 'R', 'e', 'f', 'P', 'l', 'a', 'i', 'n', 'N', 'a', 'm',
            'e'
        )
        RAPIDJSON_STRING_(
            RefInvalid, 'R', 'e', 'f', 'I', 'n', 'v', 'a', 'l', 'i', 'd'
        )
        RAPIDJSON_STRING_(
            RefPointerInvalid, 'R', 'e', 'f', 'P', 'o', 'i', 'n', 't', 'e', 'r',
            'I', 'n', 'v', 'a', 'l', 'i', 'd'
        )
        RAPIDJSON_STRING_(
            RefUnknown, 'R', 'e', 'f', 'U', 'n', 'k', 'n', 'o', 'w', 'n'
        )
        RAPIDJSON_STRING_(
            RefCyclical, 'R', 'e', 'f', 'C', 'y', 'c', 'l', 'i', 'c', 'a', 'l'
        )
        RAPIDJSON_STRING_(
            RefNoRemoteProvider, 'R', 'e', 'f', 'N', 'o', 'R', 'e', 'm', 'o',
            't', 'e', 'P', 'r', 'o', 'v', 'i', 'd', 'e', 'r'
        )
        RAPIDJSON_STRING_(
            RefNoRemoteSchema, 'R', 'e', 'f', 'N', 'o', 'R', 'e', 'm', 'o', 't',
            'e', 'S', 'c', 'h', 'e', 'm', 'a'
        )
        RAPIDJSON_STRING_(
            ReadOnlyAndWriteOnly, 'R', 'e', 'a', 'd', 'O', 'n', 'l', 'y', 'A',
            'n', 'd', 'W', 'r', 'i', 't', 'e', 'O', 'n', 'l', 'y'
        )
        RAPIDJSON_STRING_(
            RegexInvalid, 'R', 'e', 'g', 'e', 'x', 'I', 'n', 'v', 'a', 'l', 'i',
            'd'
        )
...
#undef RAPIDJSON_STRING_
        ...
        static SchemaDraft GetSchemaDraft(const ValueType &document)
        {
            ...
            if (!document.IsObject()) { return kDraftNone; }

            // Get the schema draft from the $schema keyword at the supplied
            // location
            typename ValueType::ConstMemberIterator itr =
        ...
        static OpenApiVersion GetOpenApiVersion(const ValueType &document)
        {
            ...
            if (!document.IsObject()) { return kVersionNone; }

            // Get the open api version from the swagger / openapi keyword at
        ...
        // Get the draft of the schema or the open api version (which implies
        // the draft). Report an error if schema draft or open api version not
        // supported or not recognized, or both in document, and carry on.
        void SetSchemaSpecification(const ValueType &document)
        {
            // Look for '$schema', 'swagger' or 'openapi' keyword at document
            // root
            ...
            // Use document draft or open api version if present or use spec
            // from constructor
            if (docDraft != kDraftNone) spec_ = Specification(docDraft);
            else if (docOapi != kVersionNone) spec_ = Specification(docOapi);
        ...
        void CreateSchemaRecursive(
            const SchemaType **schema, const PointerType &pointer,
            ...
            if (v.GetType() == kObjectType)
                {
                    ...
                    for (typename ValueType::ConstMemberIterator itr =
                             v.MemberBegin();
                         itr != v.MemberEnd(); ++itr)
                        CreateSchemaRecursive(
                            0, pointer.Append(itr->name, allocator_),
                ...
                for (SizeType i = 0; i < v.Size(); i++)
                    CreateSchemaRecursive(
                        0, pointer.Append(i, allocator_), v[i], document, id
                    );
        }

        // Changed by PR #1393
        const UriType &CreateSchema(
            const SchemaType **schema, const PointerType &pointer,
            const ValueType &v, const ValueType &document, const UriType &id
        )
        {
            ...
            pointer.StringifyUriFragment(sb);
            RAPIDJSON_SCHEMA_PRINT(
                Method, "GenericSchemaDocument::CreateSchema", sb.GetString(),
                id.GetString()
            );
            if (v.IsObject())
                {
                    if (const SchemaType *sc = GetSchema(pointer))
                        ...
                        {
                            // The new schema constructor adds itself and its
                            // $ref(s) to schemaMap_
                            SchemaType *s =
                                new (allocator_->Malloc(sizeof(SchemaType)))
                                    SchemaType(
        ...
        bool HandleRefSchema(
            const PointerType &source, const SchemaType **schema,
            const ValueType &v, const ValueType &document, const UriType &id
        )
        {
            typename ValueType::ConstMemberIterator itr =
            ...
            RAPIDJSON_SCHEMA_PRINT(
                Method, "GenericSchemaDocument::HandleRefSchema",
                sb.GetString(), id.GetString()
            );
            // Resolve the source pointer to the $ref'ed schema (finally)
            new (schemaRef_.template Push<SchemaRefPtr>())
                SchemaRefPtr(&source);

            if (itr->value.IsString())
                {
                    SizeType len = itr->value.GetStringLength();
                    if (len == 0) SchemaError(kSchemaErrorRefInvalid, source);
                    else
                        {
                            ...
                            // See if the resolved $ref minus the fragment
                            // matches a resolved id in this document Search
                            // from the root. Returns the subschema in the
                            // document and its absolute JSON pointer.
                            PointerType basePointer = PointerType();
                            const ValueType *base = FindId(
                                document, ref, basePointer, docId_, false
                            );
                            if (!base)
                                {
                                    // Remote reference - call the remote
                                    // document provider
                                    if (!remoteProvider_)
                                        SchemaError(
                                ...
                                { // Local reference
                                    ...
                                    if (len <= 1 || s[1] == '/')
                                        {
                                            // JSON pointer fragment, relative
                                            // to the resolved URI
                                            const PointerType relPointer(
                                                s, len, allocator_
                                            );
                                            if (!relPointer.IsValid())
                                                SchemaErrorPointer(
                                                    kSchemaErrorRefPointerInvalid,
                                                    source, s, len, relPointer
                                                );
                                            else
                                                {
                                                    // Get the subschema
                                        ...
                                        {
                                            // Plain name fragment, relative to
                                            // the resolved URI Not supported in
                                            // open api 2.0 and 3.0
                                            PointerType pointer(allocator_);
                                            if (spec_.oapi == kVersion20 ||
                                                spec_.oapi == kVersion30)
                                                SchemaErrorValue(
                                                    kSchemaErrorRefPlainName,
                                                    source, s, len
                                                );
                                            // See if the fragment matches an id
                                            // in this document. Search from the
                                            // base we just established. Returns
                                            // the subschema in the document and
        ...
        //! Find the first subschema with a resolved 'id' that matches the
        //! specified URI.
        // If full specified use all URI else ignore fragment.
        // If found, return a pointer to the subschema and its JSON pointer.
        // TODO cache pointer <-> id mapping
        ValueType *FindId(
            const ValueType &doc, const UriType &finduri, PointerType &resptr,
            ...
            if (doc.GetType() == kObjectType)
                {
                    ...
                    if (m != doc.MemberEnd() &&
                        m->value.GetType() == kStringType)
                        {
                            localuri = UriType(m->value, allocator_)
                    ...
                    for (m = doc.MemberBegin(); m != doc.MemberEnd(); ++m)
                        {
                            if (m->value.GetType() == kObjectType ||
                                ...
                                {
                                    resval = FindId(
                                        m->value, finduri, resptr, localuri,
                                        full,
                                        here.Append(
                                            m->name.GetString(),
            ...
            else if (doc.GetType() == kArrayType)
                {
                    ...
                    for (typename ValueType::ConstValueIterator v = doc.Begin();
                        ...
                        {
                            if (v->GetType() == kObjectType ||
                                v->GetType() == kArrayType)
                                {
                                    resval = FindId(
                                        *v, finduri, resptr, localuri, full,
        ...
        {
            RAPIDJSON_SCHEMA_PRINT(
                Method, "GenericSchemaDocument::AddSchemaRefs"
            );
            while (!schemaRef_.Empty())
                {
                    SchemaRefPtr *ref =
                        schemaRef_.template Pop<SchemaRefPtr>(1);
                    SchemaEntry *entry =
        ...
        internal::Stack<Allocator>
            schemaMap_; // Stores created Pointer -> Schemas
        internal::Stack<Allocator>
                                   ...
                                   typename SchemaDocumentType::SchemaType>
{
    public:
        ...
        /*!
           ...
           states. \param schemaStackCapacity Optional initial capacity of
           schema path stack. \param documentStackCapacity Optional initial
           capacity of document path stack.
        */
        GenericSchemaValidator(
            ...
            : schemaDocument_(&schemaDocument), root_(schemaDocument.GetRoot()),
              ...
              outputHandler_(0), error_(kObjectType), currentError_(),
              missingDependents_(), valid_(true), flags_(kValidateDefaultFlags),
              depth_(0)
        {
            RAPIDJSON_SCHEMA_PRINT(
                Method, "GenericSchemaValidator::GenericSchemaValidator"
            );
        }
        ...
        */
        GenericSchemaValidator(
            ...
            : schemaDocument_(&schemaDocument), root_(schemaDocument.GetRoot()),
              ...
              schemaStack_(allocator, schemaStackCapacity),
              documentStack_(allocator, documentStackCapacity),
              outputHandler_(&outputHandler), error_(kObjectType),
              currentError_(), missingDependents_(), valid_(true),
              flags_(kValidateDefaultFlags), depth_(0)
        {
            RAPIDJSON_SCHEMA_PRINT(
                Method, "GenericSchemaValidator::GenericSchemaValidator "
                        "(output handler)"
        ...
        {
            if (documentStack_.Empty()) { return PointerType(); }
            else
                {
                    return PointerType(
        ...
        {
            AddNumberError(
                kValidateErrorMultipleOf, ValueType(actual).Move(), expected
            );
        }
        ...
        {
            AddNumberError(
                exclusive ? kValidateErrorExclusiveMaximum
                          : kValidateErrorMaximum,
        ...
        {
            AddNumberError(
                exclusive ? kValidateErrorExclusiveMinimum
                          : kValidateErrorMinimum,
                ValueType(actual).Move(), expected,
        ...
        void DisallowedItem(SizeType index)
        {
            currentError_.SetObject();
            currentError_.AddMember(
                GetDisallowedString(), ValueType(index).Move(),
                GetStateAllocator()
            );
        ...
        {
            AddNumberError(
                kValidateErrorMinItems, ValueType(actualCount).Move(),
        ...
        {
            AddNumberError(
                kValidateErrorMaxItems, ValueType(actualCount).Move(),
                SValue(expectedCount).Move()
            );
        }
        ...
        {
            AddNumberError(
                kValidateErrorMaxProperties, ValueType(actualCount).Move(),
                SValue(expectedCount).Move()
            );
        }
        ...
        {
            AddNumberError(
                kValidateErrorMinProperties, ValueType(actualCount).Move(),
                SValue(expectedCount).Move()
            );
        }
        ...
        bool EndMissingProperties()
        {
            ...
            error.AddMember(
                GetMissingString(), currentError_, GetStateAllocator()
            );
            currentError_ = error;
            ...
            if (!missingDependents_.Empty())
                {
                    ...
                    error.AddMember(
                        GetMissingString(), missingDependents_.Move(),
                        GetStateAllocator()
                    );
                    ...
                    PointerType schemaRef = GetInvalidSchemaPointer().Append(
                        SchemaType::GetValidateErrorKeyword(
                            kValidateErrorDependencies
                        ),
                    ...
                    wrapper.AddMember(
                        ValueType(
                            SchemaType::GetValidateErrorKeyword(code),
                            GetStateAllocator()
                        )
                            .Move(),
        ...
        bool EndDependencyErrors()
        {
            ...
            ValueType error(kObjectType);
            error.AddMember(
                GetErrorsString(), currentError_, GetStateAllocator()
            );
            currentError_ = error;
        ...
        void EndDisallowedType(const typename SchemaType::ValueType &actualType)
        {
            ValueType error(kObjectType);
            error.AddMember(
                GetExpectedString(), currentError_, GetStateAllocator()
            );
            error.AddMember(
                GetActualString(),
        ...
        void MultipleOneOf(SizeType index1, SizeType index2)
        {
            ...
            currentError_.SetObject();
            currentError_.AddMember(
                GetMatchesString(), matches, GetStateAllocator()
            );
...
#define RAPIDJSON_STRING_(name, ...)                                           \
    ...
    {                                                                          \
        ...
        RAPIDJSON_STRING_(
            ErrorMessage, 'e', 'r', 'r', 'o', 'r', 'M', 'e', 's', 's', 'a', 'g',
            'e'
        )
        RAPIDJSON_STRING_(
    ...
    if ((!BeginValue() && !GetContinueOnErrors()) ||                           \
        ...
        {                                                                      \
            *documentStack_.template Push<Ch>() = '\0';                        \
            ...
            RAPIDJSON_SCHEMA_PRINT(                                            \
                InvalidDocument, documentStack_.template Bottom<Ch>()          \
            );                                                                 \
            valid_ = false;                                                    \
            return valid_;                                                     \
        }
    ...
    RAPIDJSON_SCHEMA_HANDLE_END_(method, arg2)
        ...
        bool RawNumber(const Ch *str, SizeType length, bool copy)
        {
            RAPIDJSON_SCHEMA_HANDLE_VALUE_(
                String, (CurrentContext(), str, length, copy),
                (str, length, copy)
            );
        }
        ...
        {
            RAPIDJSON_SCHEMA_PRINT(
                Method, "GenericSchemaValidator::StartObject"
            );
        ...
        {
            RAPIDJSON_SCHEMA_PRINT(
                Method, "GenericSchemaValidator::StartArray"
            );
...
#undef RAPIDJSON_SCHEMA_HANDLE_VALUE_
                ...
                new (GetStateAllocator().Malloc(sizeof(GenericSchemaValidator)))
                    GenericSchemaValidator(
                        *schemaDocument_, root,
                        ...
                        documentStack_.GetSize(), depth_ + 1,
                        &GetStateAllocator()
                    );
            sv->SetValidateFlags(
    ...
    private:
        ...
        {
            RAPIDJSON_SCHEMA_PRINT(
                Method, "GenericSchemaValidator::BeginValue"
            );
            ...
            else
                {
                    if (CurrentContext().inArray)
                        internal::TokenHelper<
                            internal::Stack<StateAllocator>, Ch>::
                            AppendIndexToken(
                                documentStack_,
                    ...
                    SizeType count =
                        CurrentContext().patternPropertiesSchemaCount;
                    const SchemaType **sa =
                        CurrentContext().patternPropertiesSchemas;
                    ...
                    if (count > 0)
                        {
                            ...
                            std::memset(
                                va, 0, sizeof(ISchemaValidator *) * count
                            );
                            for (SizeType i = 0; i < count; i++)
                                va[validatorCount++] = CreateSchemaValidator(
                                    *sa[i], true
                                ); // Inherit continueOnError
                        }
        ...
        bool EndValue()
        {
            ...
            *documentStack_.template Push<Ch>() = '\0';
            documentStack_.template Pop<Ch>(1);
            RAPIDJSON_SCHEMA_PRINT(
                ValidatorPointers, sb.GetString(),
                    ...
                    if (hasher && context.valueUniqueness)
                        {
                            ...
                            for (typename HashCodeArray::ConstValueIterator
                                     itr = a->Begin();
                                 itr != a->End(); ++itr)
                                if (itr->GetUint64() == h)
                                    {
        ...
        void AddErrorInstanceLocation(ValueType &result, bool parent)
        {
            ...
            result.AddMember(
                GetInstanceRefString(), instanceRef, GetStateAllocator()
            );
        }

        void AddErrorSchemaLocation(
            ValueType &result, PointerType schema = PointerType()
        )
        {
            GenericStringBuffer<EncodingType> sb;
            ...
            if (len)
                memcpy(
                    sb.Push(len), CurrentSchema().GetURI().GetString(),
                    len * sizeof(Ch)
                );
            ...
            result.AddMember(
                GetSchemaRefString(), schemaRef, GetStateAllocator()
            );
        }
        ...
        {
            typename ValueType::MemberIterator member =
                error_.FindMember(keyword);
            if (member == error_.MemberEnd())
        ...
        )
        {
            currentError_.SetObject();
            currentError_.AddMember(
                GetActualString(), actual, GetStateAllocator()
            );
            currentError_.AddMember(
                GetExpectedString(),
        ...
        void AddErrorArray(
            const ValidateErrorCode code, ISchemaValidator **subvalidators,
            SizeType count
        )
        {
            ValueType errors(kArrayType);
            ...
            currentError_.SetObject();
            currentError_.AddMember(
                GetErrorsString(), errors, GetStateAllocator()
            );
            AddCurrentError(code);
        }
