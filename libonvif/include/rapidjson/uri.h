...
class GenericUri
{
    public:
            ...
            if (this != &rhs)
                {
                    ...
                    auth_ = CopyPart(
                        scheme_, rhs.scheme_, rhs.GetSchemeStringLength()
                    );
                    path_ =
                        CopyPart(auth_, rhs.auth_, rhs.GetAuthStringLength());
                    query_ =
                        CopyPart(path_, rhs.path_, rhs.GetPathStringLength());
                    frag_ = CopyPart(
                        query_, rhs.query_, rhs.GetQueryStringLength()
                    );
                    base_ =
                        CopyPart(frag_, rhs.frag_, rhs.GetFragStringLength());
                    uri_ =
                        CopyPart(base_, rhs.base_, rhs.GetBaseStringLength());
        ...
        GenericUri Resolve(const GenericUri &baseuri, Allocator *allocator = 0)
        {
            ...
            resuri.Allocate(
                GetStringLength() + baseuri.GetStringLength() + 1
            ); // + 1 for joining slash

            if (!(GetSchemeStringLength() == 0))
                {
                    // Use all of this URI
                    resuri.auth_ = CopyPart(
                        resuri.scheme_, scheme_, GetSchemeStringLength()
                    );
                    resuri.path_ =
            ...
            else
                {
                    // Use the base scheme
                    resuri.auth_ = CopyPart(
                        resuri.scheme_, baseuri.scheme_,
                        baseuri.GetSchemeStringLength()
                    );
                    if (!(GetAuthStringLength() == 0))
                        {
                            // Use this auth, path, query
                            resuri.path_ = CopyPart(
                                resuri.auth_, auth_, GetAuthStringLength()
                            );
                            resuri.query_ = CopyPart(
                                resuri.path_, path_, GetPathStringLength()
                            );
                            resuri.frag_ = CopyPart(
                                resuri.query_, query_, GetQueryStringLength()
                            );
                            resuri.RemoveDotSegments();
                        }
                    else
                            ...
                            if (GetPathStringLength() == 0)
                            ...
                            else
                                {
                                    if (path_[0] == '/')
                                        ...
                                        {
                                            // Relative path - append this path
                                            // to base path after base path's
                                            // last slash
                                            size_t pos = 0;
                                            if (!(baseuri.GetAuthStringLength(
                                                  ) == 0) &&
                                                baseuri.GetPathStringLength() ==
                                                    0)
                                                {
    ...
    private:
        ...
        std::size_t Allocate(std::size_t len)
        {
            ...
            // Allocate one block containing each part of the URI (5) plus base
            // plus full URI, all null terminated. Order: scheme, auth, path,
            // query, frag, base, uri Note need to set, increment, assign in 3
            // stages to avoid compiler warning bug.
        ...
        // Parse a URI into constituent scheme, authority, path, query, &
        // fragment parts Supports URIs that match regex
        // ^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\?([^#]*))?(#(.*))? as per
        ...
        void Parse(const Ch *uri, std::size_t len)
        {
                            ...
                            if (pos1 < pos2)
                                {
                                    pos1++;
                                    std::memcpy(
                                        scheme_, &uri[start], pos1 * sizeof(Ch)
                                    );
                                    scheme_[pos1] = '\0';
                                    start = pos1;
                                }
            ...
            // Look for auth (//([^/?#]*))?
            // Note need to set, increment, assign in 3 stages to avoid compiler
            // warning bug.
            ...
            if (start < len - 1 && uri[start] == '/' && uri[start + 1] == '/')
                {
                    ...
                    std::memcpy(
                        auth_, &uri[start], (pos2 - start) * sizeof(Ch)
                    );
                    auth_[pos2 - start] = '\0';
                    start = pos2;
                }
            ...
            if (start < len)
                {
                    ...
                    if (start != pos2)
                        {
                            std::memcpy(
                                path_, &uri[start], (pos2 - start) * sizeof(Ch)
                            );
                            path_[pos2 - start] = '\0';
                            if (path_[0] == '/')
                                RemoveDotSegments(
            ...
            if (start < len && uri[start] == '?')
                {
                    ...
                    if (start != pos2)
                        {
                            std::memcpy(
                                query_, &uri[start], (pos2 - start) * sizeof(Ch)
                            );
                            query_[pos2 - start] = '\0';
                            start = pos2;
                        }
            ...
            while (pathpos < pathlen)
                {
                    ...
                    if (slashpos == 2 && path_[pathpos] == '.' &&
                        path_[pathpos + 1] == '.')
                        {
                            ...
                            // We expect to find a previously added slash at the
                            // end or nothing
                            RAPIDJSON_ASSERT(
                                newpos == 0 || path_[newpos - 1] == '/'
                            );
                            size_t lastslashpos = newpos;
                            // Make sure we don't go beyond the start segment
                            if (lastslashpos > 1)
                                {
                                    // Find the next to last slash and back up
                                    // to it
                                    lastslashpos--;
                                    while (lastslashpos > 0)
                                        {
