...
namespace internal
{
    ...
    {
        public:
            // Optimization note: Do not allocate memory for stack_ in
            // constructor. Do it lazily when first Push() -> Expand() ->
            // Resize().
            ...
            void ShrinkToFit()
            {
                if (Empty())
                    {
                        // If the stack is empty, completely deallocate the
                        // memory.
            ...
            // Optimization note: try to minimize the size of this function for
            // force inline. Expansion is run very infrequently, so it is moved
            // to another (probably non-inline) function.
            template <typename T>
        ...
        private:
            ...
            {
                // Only expand the capacity if the current stack exists.
                // Otherwise just create a stack with initial capacity.
                size_t newCapacity;
                if (stack_ == 0)
                    {
                        if (!allocator_)
