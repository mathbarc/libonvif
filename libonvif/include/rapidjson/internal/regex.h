...
namespace internal
{
    ...
    /*!
        ...
        \note This is a Thompson NFA engine, implemented with reference to
            Cox, Russ. "Regular Expression Matching Can Be Simple And Fast (but
       is slow in Java, Perl, PHP, Python, Ruby,...).",
    ...
    */
    ...
    class GenericRegex
    {
        ...
        private:
                        ...
                        switch (codepoint = ds.Take())
                            {
                                    ...
                                    while (
                                        ...
                                        *operatorStack.template Top<Operator>(
                                        ...
                                        if (!Eval(
                                                operandStack,
                                                *operatorStack
                                                     .template Pop<Operator>(1)
                                            ))
                                            return;
                                    *operatorStack.template Push<Operator>() =
                                        kAlternation;
                                ...
                                case '(':
                                    *operatorStack.template Push<Operator>() =
                                        kLeftParenthesis;
                                    *atomCountStack.template Push<unsigned>() =
                                        0;
                                    break;
                                ...
                                case '.':
                                    PushOperand(
                                        operandStack, kAnyCharacterClass
                                    );
                                    ImplicitConcatenation(
                                        atomCountStack, operatorStack
                                    );
                ...
                if (operandStack.GetSize() == sizeof(Frag))
                    {
                        ...
                        Patch(
                            e->out,
                            NewState(kRegexInvalidState, kRegexInvalidState, 0)
                        );
                        root_ = e->start;
                ...
                switch (op)
                    {
                        case kConcatenation:
                            RAPIDJSON_ASSERT(
                                operandStack.GetSize() >= sizeof(Frag) * 2
                            );
                            {
                        ...
                        case kAlternation:
                            if (operandStack.GetSize() >= sizeof(Frag) * 2)
                                {
                                    Frag e2 =
                                        *operandStack.template Pop<Frag>(1);
                                    Frag e1 =
                                        *operandStack.template Pop<Frag>(1);
                                    SizeType s =
                                        NewState(e1.start, e2.start, 0);
                                    *operandStack.template Push<Frag>() = Frag(
                                        s, Append(e1.out, e2.out),
                                        Min(e1.minIndex, e2.minIndex)
                                    );
                                    return true;
                                }
            ...
            bool EvalQuantifier(
                Stack<Allocator> &operandStack, unsigned n, unsigned m
            )
            {
            ...
            void CloneTopOperand(Stack<Allocator> &operandStack)
            {
                ...
                SizeType count =
                    stateCount_ -
                    src.minIndex; // Assumes top operand contains states in
            ...
            template <typename InputStream>
            bool ParseRange(
                DecodedStream<InputStream, Encoding> &ds, SizeType *range
            )
            {
                bool isBegin = true;
                bool negate = false;
                                ...
                                case ']':
                                    ...
                                    if (step == 2)
                                        { // Add trailing '-'
                                            SizeType r = NewRange('-');
                                            RAPIDJSON_ASSERT(
                                                current != kRegexInvalidRange
                                            );
                                            GetRange(current).next = r;
                                        }
                                    if (negate)
                                        GetRange(start).start |=
                                            kRangeNegationFlag;
                                    *range = start;
                                    return true;
            ...
            bool CharacterEscape(
                DecodedStream<InputStream, Encoding> &ds,
    ...
    class GenericRegexSearch
    {
        public:
                  ...
                  state0_(allocator, 0), state1_(allocator, 0), stateSet_()
            {
                ...
                stateSet_ =
                    static_cast<uint32_t *>(allocator_->Malloc(GetStateSetSize()
                    ));
            ...
            {
                return SearchWithAnchoring(
                    is, regex_.anchorBegin_, regex_.anchorEnd_
                );
            }
        ...
        private:
            ...
            bool SearchWithAnchoring(
                InputStream &is, bool anchorBegin, bool anchorEnd
            )
            {
                ...
                while (!current->Empty() && (codepoint = ds.Take()) != 0)
                    {
                        ...
                        next->Clear();
                        matched = false;
                        for (const SizeType *s =
                                 current->template Bottom<SizeType>();
                             s != current->template End<SizeType>(); ++s)
                            {
                                const State &sr = regex_.GetState(*s);
                                if (sr.codepoint == codepoint ||
                                    sr.codepoint ==
                                        RegexType::kAnyCharacterClass ||
                                    (sr.codepoint ==
                                         RegexType::kRangeCharacterClass &&
            ...
            bool AddState(Stack<Allocator> &l, SizeType index)
            {
                ...
                return s.out ==
                       kRegexInvalidState; // by using PushUnsafe() above, we
                ...
                while (rangeIndex != kRegexInvalidRange)
                    {
                        ...
                        if (codepoint >=
                                (r.start & ~RegexType::kRangeNegationFlag) &&
                            codepoint <= r.end)
                            return yes;
                        rangeIndex = r.next;
                    }
