...
namespace internal
{
    ...
    inline void GrisuRound(
        char *buffer, int len, uint64_t delta, uint64_t rest,
                ...
                if (tmp <= delta)
                    {
                        *K += kappa;
                        GrisuRound(
                            buffer, *len, delta, tmp, kPow10[kappa] << -one.e,
                            wp_w.f
                        );
                        return;
                    }
    ...
    inline char *Prettify(char *buffer, int length, int k, int maxDecimalPlaces)
    {
        ...
        if (0 <= k && kk <= 21)
        ...
        else if (0 < kk && kk <= 21)
            {
                // 1234e-2 -> 12.34
                std::memmove(
                    &buffer[kk + 1], &buffer[kk],
                    static_cast<size_t>(length - kk)
                );
                buffer[kk] = '.';
                if (0 > k + maxDecimalPlaces)
                    {
                        // When maxDecimalPlaces = 2, 1.2345 -> 1.23, 1.102
                        // -> 1.1 Remove extra trailing zeros (at least one)
                        // after truncation.
        ...
        else if (-6 < kk && kk <= 0)
            {
                ...
                if (length - kk > maxDecimalPlaces)
                    {
                        // When maxDecimalPlaces = 2, 0.123 -> 0.12, 0.102 ->
                        // 0.1 Remove extra trailing zeros (at least one) after
                        // truncation.
