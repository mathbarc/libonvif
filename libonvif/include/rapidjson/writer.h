...
enum WriteFlag {
    ...
    kWriteDefaultFlags = RAPIDJSON_WRITE_DEFAULT_FLAGS //!< Default write flags. Can be
...
public:
    ...
    /*! \param os Output stream.
        \param stackAllocator User supplied allocator. If it is null, it
       will create a private one. \param levelDepth Initial capacity of
       stack.
    */
    explicit Writer(
    ...
    /*!
        This function reset the writer with a new stream and default
       settings, in order to make a Writer object reusable for output
       multiple JSONs.
    ...
    */
    ...
    /*!
       ...
       places.
        ...
        writer.Double(1.234567890123456e30);    // "1.234567890123456e30"
       (do not truncate significand for positive exponent)
        writer.Double(1.23e-4);                 // "0.0" (do truncate
       significand for negative exponent) writer.EndArray(); \endcode
    ...
    */
    ...
    /*!
       ...
       character within [0, length - 1] range. \param length Length of the
       json. \param type Type of the root of json.
    */
    ...
    void Prefix(Type type)
    {
        ...
        if (RAPIDJSON_LIKELY(level_stack_.GetSize() != 0)) { // this value is not at root
            ...
            if (level->valueCount > 0) {
                if (level->inArray)
                    os_->Put(','); // add comma if it is not the
            ...
            if (!level->inArray && level->valueCount % 2 == 0)
                RAPIDJSON_ASSERT(
                    type == kStringType
                ); // if it's in object, then even number should be a
                   // name
            level->valueCount++;
        } else {
...
Writer<StringBuffer>::ScanWriteUnescapedString(StringStream& is, size_t length)
{
    ...
    for (; p != endAligned; p += 16) {
        ...
        _mm_storeu_si128(
            reinterpret_cast<__m128i*>(os_->PushUnsafe(16)), s
        );
    }
