...
template <typename CharType, typename InputByteStream> class AutoUTFInputStream
{
    ...
    private:
        ...
        void DetectType()
        {
            ...
            // ASCII characters [RFC0020], it is possible to determine whether
            // an octet stream is UTF-8, UTF-16 (BE or LE), or UTF-32 (BE or LE)
            // by looking at the pattern of nulls in the first four octets." 00
            // 00 00 xx  UTF-32BE 00 xx 00 xx  UTF-16BE xx 00 00 00  UTF-32LE xx
            // 00 xx 00  UTF-16LE xx xx xx xx  UTF-8

            if (!hasBOM_)
                {
                    int pattern = (c[0] ? 1 : 0) | (c[1] ? 2 : 0) |
                                  (c[2] ? 4 : 0) | (c[3] ? 8 : 0);
                    switch (pattern)
                        {
            ...
            // It only perform checks with assertion.
