        ...
        if (strlen(element)) {
            ...
            for (int i = 0; i < n; i++) {
                if (prepareOnvifData(i, onvif_session, onvif_data)) {
                    ...
                    printf("%s %s(%s)\n", host, onvif_data->host_name, onvif_data->camera_name);
                } else {
...
void profileCheck(OnvifData* onvif_data, const std::vector<std::string>& args)
{
    ...
    if (args.size() > 1) {
        ...
        if (getProfileToken(onvif_data, index))
            throw std::runtime_error(cat("get profile token - ", onvif_data->last_error));
        if (strlen(onvif_data->profileToken) == 0)
            throw std::runtime_error(cat("invalid profile token - ", (char*)std::to_string(index).c_str()).data());
                    ...
                    if (getDeviceInformation(onvif_data) == 0) {
                        ...
                        if (time_sync) {
                            ...
                            if (setSystemDateAndTime(onvif_data))
                                throw std::runtime_error(cat("set system date and time - ", onvif_data->last_error));
            ...
            if (args[0] == "get") {
                ...
                if (args[0] == "rtsp") {
                    ...
                    if (getStreamUri(onvif_data))
                        throw std::runtime_error(cat("get stream uri - ", onvif_data->last_error));
                    std::string uri(onvif_data->stream_uri);
                    if (add_pass) {
                    ...
                    if (getSnapshotUri(onvif_data))
                        throw std::runtime_error(cat("get snapshot - ", onvif_data->last_error));
                    std::string uri(onvif_data->stream_uri);
                    if (add_pass) {
                    ...
                    if (getVideoEncoderConfiguration(onvif_data))
                        throw std::runtime_error(cat("get video encoder configuration - ", onvif_data->last_error));
                    ...
                    if (getTimeOffset(onvif_data))
                        throw std::runtime_error(cat("get time offset - ", onvif_data->last_error));
                    ...
                    if (onvif_data->datetimetype != 'M') {
                        if (getNTP(onvif_data))
                            throw std::runtime_error(cat("get NTP - ", onvif_data->last_error));
                    ...
                    if (args.size() > 1) {
                        if (args[1] == "options") {
                            ...
                            if (getVideoEncoderConfigurationOptions(onvif_data))
                                throw std::runtime_error(cat("get video encoder configuration options - ", onvif_data->last_error));
                            int size = 0;
                    ...
                    if (args.size() > 1) {
                        if (args[1] == "options") {
                            ...
                            if (getOptions(onvif_data))
                                throw std::runtime_error(cat("get options - ", onvif_data->last_error));
                        ...
                        if (getImagingSettings(onvif_data))
                            throw std::runtime_error(cat("get imaging settings - ", onvif_data->last_error));
                    ...
                    if (getNetworkInterfaces(onvif_data))
                        throw std::runtime_error(cat("get network interfaces - ", onvif_data->last_error));
                    if (getNetworkDefaultGateway(onvif_data))
                        throw std::runtime_error(cat("get network default gateway - ", onvif_data->last_error));
                    ...
                    if (getStatus(onvif_data))
                        throw std::runtime_error(cat("get status - ", onvif_data->last_error));
                ...
                if (args[0] == "brightness") {
                    if (args.size() > 1) {
                        ...
                        if (setImagingSettings(onvif_data))
                            throw std::runtime_error(cat("set brightness - ", onvif_data->last_error));
                    ...
                    if (args.size() > 1) {
                        ...
                        if (setImagingSettings(onvif_data))
                            throw std::runtime_error(cat("set contrast - ", onvif_data->last_error));
                    ...
                    if (args.size() > 1) {
                        ...
                        if (setImagingSettings(onvif_data))
                            throw std::runtime_error(cat("set saturation - ", onvif_data->last_error));
                    ...
                    if (args.size() > 1) {
                        ...
                        if (setImagingSettings(onvif_data))
                            throw std::runtime_error(cat("set sharpness - ", onvif_data->last_error));
                    ...
                    if (args.size() > 1) {
                        ...
                        if (found != std::string::npos) {
                            ...
                            if (setVideoEncoderConfiguration(onvif_data))
                                throw std::runtime_error(cat("set video encoder configuration - ", onvif_data->last_error));
                        ...
                        if (args[0] == "on") {
                            ...
                            if (onvif_data->dhcp_enabled) {
                                ...
                                if (setNetworkInterfaces(onvif_data))
                                    throw std::runtime_error(cat("set network interfaces - ", onvif_data->last_error));
                    ...
                    if (args.size() > 1) {
                        ...
                        if (onvif_data->dhcp_enabled) {
                            ...
                            if (setNetworkDefaultGateway(onvif_data))
                                throw std::runtime_error(cat("set default gateway - ", onvif_data->last_error));
                    ...
                    if (args.size() > 1) {
                        ...
                        if (onvif_data->dhcp_enabled) {
                            ...
                            if (setDNS(onvif_data))
                                throw std::runtime_error(cat("set DNS - ", onvif_data->last_error));
                    ...
                    if (args.size() > 1) {
                        ...
                        if (setUser(buf, onvif_data))
                            throw std::runtime_error(cat("set user - ", onvif_data->last_error));
                ...
                if (reply == "yes") {
                    if (rebootCamera(onvif_data))
                        throw std::runtime_error(cat("reboot camera - ", onvif_data->last_error));
                ...
                if (args.size() > 1) {
                    ...
                    if (args[0] == "zone") {
                        ...
                        if (setSystemDateAndTimeUsingTimezone(onvif_data))
                            throw std::runtime_error(cat("set system date and time using timezone - ", onvif_data->last_error));
