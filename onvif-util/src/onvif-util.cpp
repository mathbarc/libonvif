...
static struct option longopts[] = { { "user", required_argument, NULL, 'u' },
...
{
    std::cout
        << "Usage: onvif-util [-ahsv] [-u <user>] [-p <password>] [command]"
        << std::endl;
}
            ...
            if (strlen(element))
                {
                    ...
                    memset(
                        onvif_session->preferred_network_address, 0,
                        sizeof(onvif_session->preferred_network_address)
                    );
                    strcpy(
                        onvif_session->preferred_network_address, ip_address
                    );

                    int n = broadcast(onvif_session);
                    std::cout << "Found " << n << " cameras on interface "
                              << ip_address << std::endl;
                    for (int i = 0; i < n; i++)
                        {
                            if (prepareOnvifData(i, onvif_session, onvif_data))
                                ...
                                {
                                    std::cout << "found invalid xaddrs in "
                                                 "device response"
                                              << std::endl;
                                }
                        }
...
static void showHelp()
{
    std::cout
        << "\n  onvif-util help\n\n"
        << "  Usage: onvif-util [-ahs] [-u <user>] [-p <password>] "
           "[command]\n\n"
        << "    options\n"
        << "      -a  poll all cameras on network and reply with host name\n"
        << "      -u  username\n"
        << "      -p  password\n"
        << "      -s  safe mode off, enable applications for viewer and "
           "browser to run\n\n"
        ...
        << "  The (n) indicates an optional profile index to apply the "
           "setting, otherwise the current profile is used.\n\n"
        << "    Data Retrieval Commands (start with get)\n\n"
        << "      get rtsp 'pass'(optional) (n) - Get rtsp uri for camera, "
           "with optional password credential\n"
        << "      get snapshot 'pass'(optional) (n) - Get snapshot uri for "
           "camera, with optional password credential\n"
        << "      get capabilities\n"
        << "      get time\n"
        << "      get profiles\n"
        << "      get profile (n)\n"
        ...
        << "      set dhcp value(required) - Accepted settings are 'on' and "
           "off'\n"
        ...
        << "      view (n) - View the camera output using ffplay (this assumes "
           "you have ffplay installed in the path\n"
...
std::string uri_with_pass(OnvifData *onvif_data)
{
    ...
    std::stringstream ss;
    ss << uri.substr(0, 7) << onvif_data->username << ":"
       << onvif_data->password << "@" << uri.substr(7);
    return ss.str();
}
    ...
    if (args.size() > 1)
        {
            ...
            if (strlen(onvif_data->profileToken) == 0)
                throw std::runtime_error(
                    cat("invalid profile token - ",
                        (char *)std::to_string(index).c_str())
                        .data()
                );
            ...
            switch (ch)
                {
                    ...
                    case 'a': showAll(); exit(0);
                    case 'h':
                        usage();
                        showHelp();
                        exit(0);
                    case 's': safe_mode = false; break;
                    case 'v':
                        std::cout << "onvif-util version " << VERSION
                                  << std::endl;
                        exit(0);
                    case 't': time_sync = true; break;
                    case 0: std::cout << optarg << std::endl; break;
                                    ...
                                    if (getDeviceInformation(onvif_data) == 0)
                                        {
                                            ...
                                            std::cout
                                                << "    serial: "
                                                << onvif_data->serial_number
                                                << "\n"
                                                << std::endl;

                                            // Initializing the session properly
                                            // with the camera requires calling
                                            // getCapabilities
                    ...
                    if (getDeviceInformation(onvif_data) == 0)
                        {
                            ...
                            if (time_sync)
                                {
                                    std::cout << "  Time sync requested"
                                              << std::endl;
                                    std::vector<std::string> tmp;
                                    profileCheck(onvif_data, tmp);
                                    if (setSystemDateAndTime(onvif_data))
                                        throw std::runtime_error(
                                            cat("set system date and time - ",
                                                onvif_data->last_error)
                                        );
                                    std::cout << "  Camera date and time has "
                                                 "been synchronized without "
                    ...
                    if (args[0] == "get")
                        {
                            ...
                            if (args[0] == "rtsp")
                                ...
                                {
                                    std::cout << "  event_service:   "
                                              << onvif_data->event_service
                                              << "\n";
                                    std::cout << "  imaging_service: "
                                              << onvif_data->imaging_service
                                              << "\n";
                                    std::cout << "  media_service:   "
                                              << onvif_data->imaging_service
                                              << "\n";
                                    std::cout << "  ptz_service:     "
                                              << onvif_data->imaging_service
                                              << "\n"
                                              << std::endl;
                                }
                            else if (args[0] == "profiles")
                                {
                                    ...
                                    while (looking)
                                        {
                                            memset(
                                                onvif_data->profileToken, 0, 128
                                            );
                                            if (getProfileToken(
                                                    onvif_data, index
                                                ))
                                                throw std::runtime_error(
                                                    cat("get profile token - ",
                                                        onvif_data->last_error)
                                                );
                                            if (strlen(onvif_data->profileToken
                                                ) == 0)
                                                looking = false;
                                            else
                                                std::cout
                            ...
                            else if (args[0] == "profile")
                                {
                                    ...
                                    if (getVideoEncoderConfiguration(onvif_data
                                        ...
                                        throw std::runtime_error(
                                            cat("get video encoder "
                                                "configuration - ",
                                                onvif_data->last_error)
                                        );
                                    std::cout
                                        << "  Width:      " << onvif_data->width
                                        << "\n";
                                    std::cout << "  Height:     "
                                              << onvif_data->height << "\n";
                                    std::cout << "  Frame Rate: "
                                            ...
                                            if (args[1] == "options")
                                                {
                                                    ...
                                                    if (getVideoEncoderConfigurationOptions(
                                                            onvif_data
                                                        ))
                                                        throw std::
                                                            runtime_error(cat(
                                            ...
                                            else
                                                {
                                                    ...
                                                    if (getVideoEncoderConfiguration(
                                                            onvif_data
                                                        ))
                                                        throw std::
                                                            runtime_error(cat(
                    ...
                    else if (args[0] == "set")
                        {
                            ...
                            if (args[0] == "brightness")
                                            ...
                                            if (found != std::string::npos)
                                                {
                                                    ...
                                                    onvif_data->width = stoi(
                                                        args[0].substr(0, found)
                                                    );
                                                    onvif_data->height =
                                                        stoi(args[0].substr(
                                                            found + 1
                                                        ));
                                                    if (setVideoEncoderConfiguration(
                                                            onvif_data
                                                        ))
                                                        throw std::
                                                            runtime_error(cat(
                                    ...
                                    if (args.size() > 1)
                                        {
                                            ...
                                            if (setVideoEncoderConfiguration(
                                                ...
                                                throw std::runtime_error(
                                                    cat("set video encoder "
                                                        "configuration - ",
                                                        onvif_data->last_error)
                                                );
                                            std::cout
                                                << "  Gov Length was set to "
                                                << value << "\n"
                                                << std::endl;
                                        }
                                    else
                                ...
                                {
                                    std::cout << "  Unrecognized command, use "
                                                 "onvif-util -h to see help\n"
                                              << std::endl;
                                }
                        }
                    else if (args[0] == "move")
                            ...
                            if (args[0] == "pan")
                                {
                                    if (args.size() > 2)
                                        {
                                            args.erase(args.begin());
                                            double const pan_rate =
                                                stof(args[0]);
                                            double const tilt_rate =
                                                stof(args[1]);

                                            if (continuousMove(
                                                    pan_rate, tilt_rate, 0,
                                                    onvif_data
                                                ))
                                                throw std::runtime_error(
                                                    cat("move pan - ",
                            ...
                            else if (args[0] == "zoom")
                                {
                                    if (args.size() > 1)
                                        {
                                            args.erase(args.begin());
                                            double const zoom_rate =
                                                stof(args[0]);

                                            if (continuousMove(
                                                    0, 0, zoom_rate, onvif_data
                                                ))
                                                throw std::runtime_error(
                                                    cat("move zoom - ",
                                                        onvif_data->last_error)
                        ...
                        {
                            std::cout << "  Are you sure you want to reboot?  "
                                         "Type yes to confirm\n"
                                      << std::endl;
                            std::string reply;
                            ...
                            if (reply == "yes")
                                ...
                                {
                                    std::cout << "  Confirmation not received, "
                                                 "reboot cancelled\n"
                                              << std::endl;
                                }
                        }
                            ...
                            if (args.size() > 1)
                                {
                                    ...
                                    if (args[0] == "zone")
                                        {
                                            profileCheck(onvif_data, args);
                                            if (setSystemDateAndTimeUsingTimezone(
                                                    onvif_data
                                                ))
                                                throw std::runtime_error(cat(
                                            ...
                                            std::cout
                                                << "  Camera date and time has "
                                                   "been synchronized using "
                                                   "the camera timezone\n"
                                                << std::endl;
                                        }
                                }
                        ...
                        {
                            safe_mode = false;
                            std::cout << "  Safe mode has been turned off, run "
                                         "only known safe apps and cameras\n"
                                      << std::endl;
                        }
                    else if (args[0] == "view")
                        {
                            if (safe_mode)
                                {
                                    std::cout
                                        << "  SAFE MODE ON, use 'unsafe' "
                                           "command to disable safe mode, or "
                                           "-s option from command line\n"
                                        << std::endl;
                                    continue;
                                }
                            ...
                            else { profileCheck(onvif_data, args); }
                            if (getStreamUri(onvif_data))
                                throw std::runtime_error(cat(
                                    "get stream uri - ", onvif_data->last_error
                                ));
                            std::stringstream ss;
#ifdef _WIN32
                            ss << "start " << player << " \""
                               << uri_with_pass(onvif_data) << "\"";
#else
